...
import Bcc.Ledger.Serialization (decodeRecordSum, ratioFromCBOR, ratioToCBOR)
import Bcc.Prelude (NFData, cborError)
...
import Sophie.Spec.NonIntegral (ln')
...
-- "Shardagnostic Optimum: An adaptively-secure, semi-synchronous proof-of-stake protocol"
--------------------------------------------------------------------------------
...
data Globals = Globals
  { epochInfoWithErr :: !(EpochInfo (Either Text)),
    ...
    maxMajorPV :: !Natural,
    -- | Maximum number of entropic in the system
...
type SophieBase = ReaderT Globals Identity
