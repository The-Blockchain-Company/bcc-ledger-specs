...
module Bcc.Ledger.Keys
  ( KeyRole (..),
    ...
    -- * Re-exports from bcc-crypto-class
...
import Bcc.Ledger.Crypto (ADDRHASH, Crypto, DSIGN, HASH, KES, VRF)
...
--------------------------------------------------------------------------------
-- Vested and Vested delegates
-------------------------------------------------------------------------------
data VestedDelegPair crypto = VestedDelegPair
  { vestedDelegKeyHash :: !(KeyHash 'VestedDelegate crypto),
    vestedDelegVrfHash :: !(Hash crypto (VerKeyVRF crypto))
  }
  deriving (Show, Eq, Ord, Generic)

instance NoThunks (VestedDelegPair crypto)

instance NFData (VestedDelegPair crypto)

instance Crypto crypto => ToCBOR (VestedDelegPair crypto) where
  toCBOR (VestedDelegPair ak vrf) =
    encodeListLen 2 <> toCBOR ak <> toCBOR vrf

instance Crypto crypto => FromCBOR (VestedDelegPair crypto) where
  fromCBOR = do
    decodeRecordNamed
      "VestedDelegPair"
      (const 2)
      (VestedDelegPair <$> fromCBOR <*> fromCBOR)

instance Crypto crypto => ToJSON (VestedDelegPair crypto) where
  toJSON (VestedDelegPair ad av) =
    Aeson.object
      [ "vesteddelegate" .= ad,
        "vrf" .= av
      ]

instance Crypto crypto => FromJSON (VestedDelegPair crypto) where
  parseJSON =
    Aeson.withObject "VestedDelegPair" $ \obj ->
      VestedDelegPair
        <$> obj .: "vesteddelegate"
        <*> obj .: "vrf"

newtype VestedDelegs crypto = VestedDelegs
  { unVestedDelegs :: Map (KeyHash 'Vested crypto) (VestedDelegPair crypto)
  }
  deriving (Eq, FromCBOR, NoThunks, NFData, Generic)
  deriving (Show) via Quiet (VestedDelegs crypto)

deriving instance
  (Crypto crypto) =>
  ToCBOR (VestedDelegs crypto)

newtype AKeys crypto = AKeys {unAKeys :: Set (VKey 'Vested crypto)}
  deriving (Eq, NoThunks, Generic)
  deriving (Show) via Quiet (AKeys crypto)
--------------------------------------------------------------------------------
-- crypto-parametrised types
--
-- Within `bcc-ledger-specs`, we parametrise everything on our `crypto` type
-- "package". However, in `bcc-crypto-class`, things are parametrised on the
