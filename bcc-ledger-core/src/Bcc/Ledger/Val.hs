...
import Bcc.Binary (Decoder, Encoding, decodeWord64, toCBOR)
...
class
  ...
  -- | Get the BCC present in the value (since BCC is our "blessed" currency)
  coin :: t -> Coin

  -- | Create a value containing only this amount of BCC
  inject :: Coin -> t
...
-- returns a Value containing only the coin (bcc) tokens from the input Value
bccOnly :: Val v => v -> Bool
bccOnly v = (inject . coin) v == v
