...
-- | Support for multiple (Sophie-based) eras in the ledger.
module Bcc.Ledger.Era
  ( Era,
    Crypto,
    PreviousEra,
    TranslationContext,
...
{-# DEPRECATED TxInBlock "Please use Bcc.Ledger.Core.Tx instead." #-}
...
-- | Translation of types between eras, e.g., from Sophie to Evie.
--
-- When @era@ is just a phantom type parameter, an empty standalone deriving can be used:
--
-- > newtype Foo era = Foo Int
-- >
-- > instance TranslateEra (Evie c) Foo
--
-- Note that one could use @DerivingAnyClass@ (@deriving (TranslateEra (Evie
...
-- >
-- > instance CryptoClass.Crypto c => TranslateEra (Evie c) Bar where
-- >     translateEra ctxt = Bar <$> translateEra ctxt
-- >
-- > -- With the following instance being in scope:
-- > instance CryptoClass.Crypto c => TranslatEra (Evie c) TxBody
--
...
in files that cause circular import dependencies.
   -- import Sophie.Spec.Ledger.TxBody(DCert,Wdrl,WitVKey)
   -- import Sophie.Spec.Ledger.Tx(TxIn)
These would have to be moved into a module such as Bcc.Ledger.TxBase(TxIn,DCert,Wdrl)
