...
module Test.Bcc.Ledger.Properties where

import Bcc.Ledger.Aurum (AurumEra, Value)
import Bcc.Ledger.Aurum.Data (Data, DataHash, hashData)
import Bcc.Ledger.Aurum.Language (Language (..))
import Bcc.Ledger.Aurum.PParams (PParams, PParams' (..))
...
import Bcc.Ledger.SafeHash (SafeHash, hashAnnotated)
...
import Sophie.Spec.Ledger.TxBody (DCert (..), DelegCert (..), Delegation (..), PoolParams (..))
import Sophie.Spec.Ledger.UTxO (balance, makeWitnessVKey)
...
data GenState = GenState
  { gsKeys :: Map (KeyHash 'Witness C_Crypto) (KeyPair 'Witness C_Crypto),
    ...
    gsZerepochScripts :: Map (ScriptHash C_Crypto, Tag) (IsValid, Script A),
      ...
      lookupScript scriptHash mTag >>= \case
        ...
        Just script -> do
          ...
          case script of
            ...
            ZerepochScript _ps -> pure $ const scriptWit
    mkTimelockWit =
      \case
...
genZerepochScript :: Tag -> GenRS (ScriptHash C_Crypto)
genZerepochScript tag = do
  isValid <- lift $ frequency [(5, pure False), (95, pure True)]
  -- Zerepoch scripts alwaysSucceeds needs at least numArgs, while
  ...
  let scriptHash = hashScript @A script
  modify $ \ts@GenState {gsZerepochScripts} ->
    ts {gsZerepochScripts = Map.insert (scriptHash, tag) (IsValid isValid, script) gsZerepochScripts}
  pure scriptHash

genScript :: Tag -> GenRS (ScriptHash C_Crypto)
genScript tag = elementsT [genTimelockScript, genZerepochScript tag]
...
lookupScript scriptHash mTag = do
  ...
  case Map.lookup scriptHash m of
    ...
    Nothing
      | Just tag <- mTag ->
        Just . snd <$> lookupByKeyM "zerepochScript" (scriptHash, tag) gsZerepochScripts
    _ -> pure Nothing

lookupZerepochScript ::
  MonadState GenState m =>
  Credential k C_Crypto ->
  Tag ->
  m (Maybe (IsValid, ScriptHash C_Crypto))
lookupZerepochScript (KeyHashObj _) _ = pure Nothing
lookupZerepochScript (ScriptHashObj scriptHash) tag =
  fmap (Map.lookup (scriptHash, tag) . gsZerepochScripts) get <&> \case
    Nothing -> Nothing
  ...
  let creds =
   ...
   in fmap (first allValid . unzip . catMaybes) $
        forM creds $ \(ix, genDat, cred) ->
          lookupZerepochScript cred tag >>= \case
...
-- zerepoch scripts), while occasianally picking out randomly from previously
-- generated set.
...
genCredential tag =
  ...
  where
    ...
    pickExistingScript =
      ScriptHashObj
        <$> elementsT [pickExistingZerepochScript, pickExistingTimelockScript]
    pickExistingZerepochScript = do
      zerepochScriptsMap <-
        Map.filterWithKey (\(_, t) _ -> t == tag) . gsZerepochScripts <$> get
      lift (genMapElem zerepochScriptsMap) >>= \case
        Just ((h, _), _) -> pure h
    ...
    case cred of
      ...
      ScriptHashObj scriptHash ->
        lookupScript scriptHash (Just Spend) >>= \case
          Just (ZerepochScript _) -> SJust . fst <$> genDatumWithHash
          _ -> pure SNothing
...
genDCerts = do
  let genUniqueScript (!dcs, !ss, !regCreds) _ = do
        dc <- genDCert
        -- Workaround a misfeature where duplicate zerepoch scripts in DCert are ignored
              ...
              if deregCred `Set.member` regCreds
                then
                  insertIfNotPresent dcs (Set.delete deregCred regCreds) Nothing
                    <$> lookupZerepochScript deregCred Cert
            ...
            | Delegate (Delegation delegCred delegKey) <- d ->
              let (dcs', regCreds')
               ...
               in insertIfNotPresent dcs' regCreds' (Just delegKey)
                    <$> lookupZerepochScript delegCred Cert
...
genValidatedTx = do
  ...
  -- 2. Check if all Zerepoch scripts are valid
  ...
  -- 3. Generate all recipients and witnesses needed for spending Zerepoch scripts
...
genTxAndUTXOState :: Gen (TRC (AurumUTXOW A), GenState)
...
genTxAndLEDGERState :: Gen (TRC (AurumLEDGER A), GenState)
  ...
  let genT = do
      ...
      pp =
        def
          { _minfeeA = minfeeA,
            _minfeeB = minfeeB,
            _costmdls = Map.singleton ZerepochV1 $ CostModel $ 0 <$ fromJust defaultCostModelParams,
            _maxValSize = 1000,
...
totalBcc :: UTxOState A -> DPState C_Crypto -> Coin
totalBcc (UTxOState utxo f d _) DPState {_dstate} =
  f <> d <> coin (balance utxo) <> F.fold (_rewards _dstate)

testTxValidForLEDGER :: (TRC (AurumLEDGER A), GenState) -> Property
testTxValidForLEDGER (trc@(TRC (_, (utxoState, dpstate), vtx)), _) =
  case runSophieBase $ applySTSTest trc of
    Right (utxoState', dpstate') ->
      classify (coerce (isValid vtx)) "TxValid" $
        totalBcc utxoState' dpstate' === totalBcc utxoState dpstate
    Left e -> counterexample (show e) (property False)

aurumProperties :: TestTree
aurumProperties =
  testGroup
    "Aurum UTXOW property tests"
