...
module Bcc.Protocol.TOptimum.Rules.Overlay
  ( OVERLAY,
    PredicateFailure,
    OverlayEnv (..),
...
import Bcc.Protocol.TOptimum.Rules.OCert (OCERT, OCertEnv (..))
...
data OverlayPredicateFailure crypto
  ...
  | VRFLeaderValueTooBig
      ...
      !ActiveSlotCoeff -- Optimum active slot coefficient value
...
instance
  ...
  type BaseM (OVERLAY crypto) = SophieBase
...
optimumVrfChecks ::
  ...
  BHBody crypto ->
  Either (PredicateFailure (OVERLAY crypto)) ()
optimumVrfChecks eta0 (PoolDistr pd) f bhb = do
...
pbftVrfChecks vrfHK eta0 bhb = do
  unless
    (vrfHK == hashVerKeyVRF vrfK)
    (throwError $ WrongGenesisVestedVRFKeyOVERLAY hk vrfHK (hashVerKeyVRF vrfK))
  vrfChecks eta0 bhb
  pure ()
  where
  ...
  judgmentContext
    >>= \( TRC
        ...
        case (lookupInOverlaySchedule firstSlotNo gkeys dval asc slot :: Maybe (OBftSlot crypto)) of
          Nothing ->
            optimumVrfChecks eta0 pd asc bhb ?!: id
          Just NonActiveSlot ->
            failBecause $ NotActiveSlotOVERLAY (bheaderSlotNo bhb)
          Just (ActiveSlot gkey) ->
            case Map.lookup gkey genDelegs of
              Nothing ->
                failBecause $ UnknownGenesisVestedKeyOVERLAY gkey
              Just (GenDelegPair genDelegsKey genesisVrfKH) -> do
                vkh == coerceKeyRole genDelegsKey ?! WrongGenesisVestedColdKeyOVERLAY vkh genDelegsKey
