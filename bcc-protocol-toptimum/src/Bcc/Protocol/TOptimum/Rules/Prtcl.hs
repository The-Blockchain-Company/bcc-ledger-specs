...
module Bcc.Protocol.TOptimum.Rules.Prtcl
  ( PRTCL,
    State,
    PrtclEnv (..),
    PrtclState (..),
...
import Bcc.Ledger.Crypto (Crypto, VRF)
...
import Bcc.Protocol.TOptimum.Rules.Overlay (OVERLAY, OverlayEnv (..))
import Bcc.Protocol.TOptimum.Rules.Updn (UPDN, UpdnEnv (..), UpdnState (..))
...
instance
  ...
  type BaseM (PRTCL crypto) = SophieBase
