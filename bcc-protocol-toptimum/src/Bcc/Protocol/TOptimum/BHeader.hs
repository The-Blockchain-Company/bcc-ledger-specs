...
-- We are enabling orphans since BlockNo is orphaned until the instance is added to bcc-base
-- https://github.com/The-Blockchain-Company/bcc-base/pull/233
{-# OPTIONS_GHC -Wno-orphans #-}

module Bcc.Protocol.TOptimum.BHeader
  ( HashHeader (..),
    PrevHash (..),
    BHeader (BHeader),
    BHBody (..),
...
import Bcc.Protocol.TOptimum.OCert (OCert (..))
...
import Sophie.Spec.NonIntegral (CompareResult (..), taylorExpCmp)
...
prevHashToNonce = \case
  GenesisHash -> NeutralNonce -- This case can only happen when starting Sophie from genesis,
