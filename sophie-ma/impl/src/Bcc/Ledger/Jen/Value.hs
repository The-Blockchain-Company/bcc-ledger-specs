...
import Bcc.Ledger.Coin (Coin (..), integerToWord64)
...
import Bcc.Ledger.Serialization (decodeMap, encodeMap)
...
instance CC.Crypto crypto => Val (Value crypto) where
  ...
  size vv@(Value _ v)
    -- when Value contains only bcc
    -- !WARNING! This branch is INCORRECT in the Jen era and should ONLY be
    -- used in the Aurum ERA.
    -- TODO - find a better way to reconcile the mistakes in Jen with what needs
    -- to be the case in Aurum.
    | v == mempty = 2
    -- when Value contains bcc as well as other tokens
    -- sums up :
    -- i) bccWords : the space taken up by the bcc amount
    -- ii) numberMulAssets : the space taken by number of words used to store
    --    number of non-bcc assets in a value
...
-- space (in Word64s) taken up by the bcc amount
bccWords :: Int
bccWords = 1
...
repOverhead :: Int
repOverhead = 4 + bccWords + numberMulAssets
...
data CompactValue crypto
  = CompactValueBccOnly {-# UNPACK #-} !Word64
...
to (Value bcc ma)
  | Map.null ma =
    CompactValueBccOnly <$> integerToWord64 bcc
to v = do
  c <- integerToWord64 bcc
...
from (CompactValueBccOnly c) = Value (fromIntegral c) mempty
...
-- of policyID, asset name and quantity, plus separately the bcc quantity.
