...
module Bcc.Ledger.SophieMA.TxBody
  ( TxBody
      ( TxBody,
        TxBodyConstr,
        TxBody',
...
import Bcc.Ledger.BaseTypes (StrictMaybe (SJust, SNothing), isSNothing)
import Bcc.Ledger.Coin (Coin (..))
import Bcc.Ledger.Core (PParamsDelta, Script, Value)
...
import Bcc.Ledger.SafeHash (HashAnnotated, SafeToHash)
import Bcc.Ledger.Serialization (encodeFoldable)
...
-- concerns as we want the Sophie era TxBody to deserialise as a Sophie-ma TxBody.
...
-- import Bcc.Ledger.SophieMA.TxBody(TxBodyRaw(..))     OR
-- import qualified Bcc.Ledger.SophieMA.TxBody as XXX
