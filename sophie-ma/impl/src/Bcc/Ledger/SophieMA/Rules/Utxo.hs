...
module Bcc.Ledger.SophieMA.Rules.Utxo where

import Bcc.Binary (FromCBOR (..), ToCBOR (..), encodeListLen, serialize)
...
making a Sophie-sized UTxO entry", calculated here by "utxoEntrySizeWithoutVal + uint",
using the constants in the "where" clause.
In the case when a UTxO entry contains coins only (and the Sophie
...
for the number of Sophie UTxO-entry bytes) by the size of a Sophie UTxO entry.
...
scaledMinDeposit v (Coin mv)
  ...
  where
    ...
    -- how much bcc does a Word64 of UTxO space cost, calculated from minBccValue PP
    -- round down
...
deriving stock instance
  ( Sophie.TransUTxOState Show era,
    TransValue Show era,
...
--
--   This differs from the corresponding Sophie function @Sophie.consumed@
--   since it also considers the "mint" field which creates or destroys non-Bcc
--   tokens.
--
--   Note that this is slightly confusing, since it also covers non-Bcc assets
...
consumed pp u tx =
  ...
  where
    ...
    refunds = Sophie.keyRefunds pp tx
    withdrawals = fold . unWdrl $ getField @"wdrls" tx

-- | The UTxO transition rule for the Sophie-MA (Jen and Evie) eras.
utxoTransition ::
  forall era.
  ( UsesTxBody era,
    UsesTxOut era,
...
utxoTransition = do
  TRC (Sophie.UtxoEnv slot pp stakepools genDelegs, u, tx) <- judgmentContext
  let Sophie.UTxOState utxo deposits' fees ppup = u
  ...
  let minFee = Sophie.minfee pp tx
  ...
  -- Check that the mint field does not try to mint BCC. This is equivalent to
  -- the check `bccPolicy ∉ supp mint tx` in the spec.
  ...
  let refunded = Sophie.keyRefunds pp txb
  let txCerts = toList $ getField @"certs" txb
  let depositChange = totalDeposits pp (`Map.notMember` stakepools) txCerts Val.<-> refunded

  pure
    Sophie.UTxOState
      { Sophie._utxo = eval ((txins @era txb ⋪ utxo) ∪ txouts txb),
        Sophie._deposited = deposits' <> depositChange,
        Sophie._fees = fees <> getField @"txfee" txb,
        Sophie._ppups = ppup'
      }
...
instance
  ...
  STS (UTXO era)
  where
  type State (UTXO era) = Sophie.UTxOState era
  type Signal (UTXO era) = Tx era
  type Environment (UTXO era) = Sophie.UtxoEnv era
  type BaseM (UTXO era) = SophieBase
...
instance
  ( TransValue ToCBOR era,
    Sophie.TransUTxOState ToCBOR era,
  ...
  toCBOR = \case
    ...
    OutputBootAddrAttrsTooBig outs ->
      encodeListLen 2 <> toCBOR (10 :: Word8)
        <> encodeFoldable outs
    TriesToForgeBCC -> encodeListLen 1 <> toCBOR (11 :: Word8)
