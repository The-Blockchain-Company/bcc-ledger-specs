...
-- | Export the EraGen instance for JenEra, as well as some reusable functions for future Eras
module Test.Bcc.Ledger.JenEraGen (genMint, jenGenesisValue, policyIndex, addTokens) where

import Bcc.Ledger.AuxiliaryData (AuxiliaryDataHash)
import Bcc.Ledger.BaseTypes (StrictMaybe (..))
import Bcc.Ledger.Coin (Coin (..))
import qualified Bcc.Ledger.Core as Core (AuxiliaryData, PParams, TxOut, Value)
...
import Bcc.Ledger.SophieMA.Rules.Utxo (scaledMinDeposit)
import Bcc.Ledger.SophieMA.Timelocks (Timelock (..))
import Bcc.Ledger.SophieMA.TxBody (TxBody (TxBody))
...
import Sophie.Spec.Ledger.Tx (TxIn, TxOut (..), hashScript, pattern Tx, pattern WitnessSet)
import Sophie.Spec.Ledger.TxBody (DCert, Wdrl)
...
import Test.Bcc.Ledger.EraBuffet (JenEra)
...
import Test.Sophie.Spec.Ledger.Generator.Core (GenEnv (..), genInteger)
...
{------------------------------------------------------------------------------
 ...
 Sophie property tests for (JenEra crypto)

 This instance is layered on top of the SophieMA instances
 in Bcc.Ledger.SophieMA.Scripts:

   `type instance Core.Script (JenEra c) = Timelock (JenEra c)`
   `instance ValidateScript (SophieMAEra ma c) where ... `
------------------------------------------------------------------------------}

instance (CryptoClass.Crypto c) => ScriptClass (JenEra c) where
  isKey _ (RequireSignature hk) = Just hk
  isKey _ _ = Nothing
  basescript _proxy = someLeaf @(JenEra c)
  quantify _ = quantifyTL
  unQuantify _ = unQuantifyTL

instance (CryptoClass.Crypto c, Mock c) => EraGen (JenEra c) where
...
genAuxiliaryData ::
  Mock crypto =>
  Constants ->
  Gen (StrictMaybe (Core.AuxiliaryData (JenEra crypto)))
...
jenGenesisValue :: forall era crypto. CryptoClass.Crypto crypto => GenEnv era -> Gen (Value crypto)
jenGenesisValue (GenEnv _ _ Constants {minGenesisOutputVal, maxGenesisOutputVal}) =
...
redCoinId = PolicyID $ hashScript @(JenEra c) redCoins
...
blueCoinId = PolicyID $ hashScript @(JenEra c) blueCoins
...
yellowCoinId = PolicyID $ hashScript @(JenEra c) yellowCoins
...
-- It will add them to the first output that has enough entropic
...
addTokens proxy tooLittleEntropic pparams ts (txout :<| os) =
  let v = getField @"value" txout
   in if Val.coin v < scaledMinDeposit v (getField @"_minUTxOValue" pparams)
        then addTokens proxy (txout :<| tooLittleEntropic) pparams ts os
        else Just $ tooLittleEntropic >< addValToTxOut @era ts txout <| os
addTokens _proxy _ _ _ StrictSeq.Empty = Nothing

-- | This function is only good in the Jen Era
genTxBody ::
  forall era.
  ( EraGen era,
    Core.Value era ~ Value (Crypto era),
...
instance Mock c => MinGenTxout (JenEra c) where
