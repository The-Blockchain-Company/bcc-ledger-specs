...
module Bcc.Ledger.Aurum.Scripts
  ( Tag (..),
    Script (TimelockScript, ZerepochScript),
    txscriptfee,
    ppTag,
    ppScript,
    isZerepochScript,
...
import Bcc.Binary (DecoderError (..), FromCBOR (fromCBOR), ToCBOR (toCBOR), serialize')
...
import Bcc.Ledger.Era (Era (Crypto), ValidateScript (hashScript))
...
import Zerepoch.V1.Ledger.Api (defaultCostModelParams, validateCostModelParams)
...
-- | Scripts in the Aurum Era, Either a Timelock script or a Zerepoch script.
data Script era
  = TimelockScript (Timelock (Crypto era))
  | ZerepochScript ShortByteString -- A Zerepoch.V1.Ledger.Scripts(Script) that has been 'CBOR' encoded
...
instance (ValidateScript era, Core.Script era ~ Script era) => Show (Script era) where
  ...
  show s@(ZerepochScript _) = "ZerepochScript " ++ show (hashScript @era s)
...
instance SafeToHash (Script era) where
  ...
  originalBytes (ZerepochScript bs) = fromShort bs

alwaysSucceeds, alwaysFails :: Natural -> Script era
alwaysSucceeds n = ZerepochScript (Zerepoch.alwaysSucceedingNAryFunction n)
alwaysFails n = ZerepochScript (Zerepoch.alwaysFailingNAryFunction n)

isZerepochScript :: Script era -> Bool
isZerepochScript (ZerepochScript _) = True
isZerepochScript (TimelockScript _) = False
...
-- we use the 'canonical' serialization approach used in Cole.
...
{-# DEPRECATED defaultCostModel "Use 'import Test.Bcc.Ledger.Aurum.ZerepochScripts' instead." #-}
...
decodeCostModel ZerepochV1 =
  case defaultCostModelParams of
    Nothing -> fail "Default Zerepoch Cost Model is corrupt."
    Just dcm -> do
...
encodeScript (ZerepochScript s) = Sum ZerepochScript 1 !> To s -- Use the ToCBOR instance of ShortByteString

instance
  (CC.Crypto (Crypto era), Typeable (Crypto era), Typeable era) =>
  FromCBOR (Annotator (Script era))
  where
  fromCBOR = decode (Summands "Aurum Script" decodeScript)
    where
      decodeScript :: Word -> Decode 'Open (Annotator (Script era))
      decodeScript 0 = Ann (SumD TimelockScript) <*! From
      decodeScript 1 = Ann (SumD ZerepochScript) <*! Ann From
...
ppScript s@(ZerepochScript _) = ppString "ZerepochScript " PP.<+> ppScriptHash (hashScript @era s)
