...
module Bcc.Ledger.Aurum.PParams
  ( PParams' (..),
    PParams,
    emptyPParams,
    ProtVer (..),
...
import Bcc.Ledger.Aurum.Language (Language (ZerepochV1), ppLanguage)
...
import Sophie.Spec.Ledger.PParams (HKD, ProtVer (..))
import qualified Sophie.Spec.Ledger.PParams as Sophie (PParams' (..))

type PParamsUpdate era = PParams' StrictMaybe era

-- | Protocol parameters.
-- Sophie parameters + additional ones
data PParams' f era = PParams
  { -- | The linear factor for the minimum fee calculation
    ...
    -- new/updated for aurum

    -- | Cost in entropic per word (8 bytes) of UTxO storage (instead of _minUTxOValue)
...
getLanguageView ::
  ...
  Language ->
  LangDepView
getLanguageView pp lang@ZerepochV1 =
  LangDepView
    (serialize' lang)
...
-- | Turn an PParams' into a Sophie.Params'
retractPP :: HKD f Coin -> PParams' f era2 -> Sophie.PParams' f era1
retractPP
  c
  (PParams ma mb mxBB mxT mxBH kd pd emx a n rho tau d eE pv mnP _ _ _ _ _ _ _ _) =
    Sophie.PParams ma mb mxBB mxT mxBH kd pd emx a n rho tau d eE pv c mnP

-- | Given the missing pieces Turn a Sophie.PParams' into an Params'
extendPP ::
  Sophie.PParams' f era1 ->
  HKD f Coin ->
  ...
  PParams' f era2
extendPP
  (Sophie.PParams ma mb mxBB mxT mxBH kd pd emx a n rho tau d eE pv _ mnP)
  bcc
  cost
  price
  mxTx
  ...
  col
  mxCol =
    PParams ma mb mxBB mxT mxBH kd pd emx a n rho tau d eE pv mnP bcc cost price mxTx mxBl mxV col mxCol

-- ======================================================
-- Pretty instances

ppPParams :: PParams' Identity era -> PDoc
ppPParams (PParams feeA feeB mbb mtx mbh kd pd em no a0 rho tau d ex pv mpool bcc cost prices mxEx mxBEx mxV c mxC) =
  ppRecord
    "PParams"
...
ppPParamsUpdate (PParams feeA feeB mbb mtx mbh kd pd em no a0 rho tau d ex pv mpool bcc cost prices mxEx mxBEx mxV c mxC) =
  ppRecord
    "PParams"
