...
module Bcc.Ledger.Aurum.TxBody
...
import Bcc.Ledger.Aurum.Data (AuxiliaryDataHash (..), DataHash)
...
import Bcc.Ledger.Jen.Value (Value (..), policies, policyID, ppValue)
...
import Bcc.Ledger.SophieMA.Timelocks (ValidityInterval (..), ppValidityInterval)
...
import Sophie.Spec.Ledger.CompactAddr (CompactAddr, compactAddr, decompactAddr)
...
import Sophie.Spec.Ledger.TxBody (TxIn (..), Wdrl (Wdrl), unWdrl)
...
data TxBodyRaw era = TxBodyRaw
  { _inputs :: !(Set (TxIn (Crypto era))),
    ...
    -- Bcc.Ledger.Jen.Value.Value, not a Core.Value.
    -- Operations on the TxBody in the AurumEra depend upon this.
...
-- the record syntax in the TxBody pattern, they inherit the (AurumBody era)
...
instance (Crypto era ~ c) => HasField "mint" (TxBody era) (Jen.Value c) where
