...
module Bcc.Ledger.Aurum.Rules.Utxos
  ( UTXOS,
...
import Bcc.Ledger.Aurum.ZerepochScriptApi
  ( CollectError,
...
import Bcc.Ledger.Rules.ValidationMode (lblStatic)
import Bcc.Ledger.Val as Val
...
import Sophie.Spec.Ledger.LedgerState (PPUPState (..), UTxOState (..), keyRefunds)
...
import Sophie.Spec.Ledger.UTxO (balance, totalDeposits)
...
instance
  ...
  STS (UTXOS era)
  where
  type BaseM (UTXOS era) = SophieBase
...
data UtxosPredicateFailure era
  ...
  | -- | We could not find all the necessary inputs for a Zerepoch Script.
...
constructValidated ::
  ...
  ( MonadError [UtxosPredicateFailure era] m,
    ...
    Core.Value era ~ Value (Crypto era),
    Core.TxBody era ~ Aurum.TxBody era,
    Core.Witnesses era ~ Aurum.TxWitness era,
    ValidateScript era,
    HasField "inputs" (Core.TxBody era) (Set (TxIn (Crypto era))),
    HasField "certs" (Core.TxBody era) (StrictSeq (DCert (Crypto era))),
    HasField "datahash" (Core.TxOut era) (StrictMaybe (DataHash (Crypto era))),
...
-- $2-phase
--
-- Above and beyond 'static' checks (see 'Bcc.Ledger.Rules.ValidateMode') we
