...
import Bcc.Ledger.Aurum.Tx (IsValid (..), ValidatedTx (..), ppTx, segwitTx)
...
import Bcc.Ledger.SafeHash (SafeToHash, originalBytes)
...
import Sophie.Spec.Ledger.BlockChain (constructMetadata)
...
ppTxSeq (TxSeq' xs _ _ _ _) =
  ppSexp "Aurum TxSeq" [ppStrictSeq ppTx xs]
