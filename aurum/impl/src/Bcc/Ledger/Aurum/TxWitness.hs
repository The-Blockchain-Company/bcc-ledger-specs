...
module Bcc.Ledger.Aurum.TxWitness
  ( RdmrPtr (..),
    Redeemers
      ( Redeemers,
        Redeemers'
...
import Bcc.Ledger.Aurum.Data (Data, DataHash, hashData, ppData)
import Bcc.Ledger.Aurum.Scripts (ExUnits (..), Script (..), Tag, isZerepochScript, ppExUnits, ppTag)
...
import Bcc.Ledger.Serialization (FromCBORGroup (..), ToCBORGroup (..))
...
encodeWitnessRaw vkeys boots scripts dats rdmrs =
  Keyed
    ...
    !> Omit
      null
      (Key 3 $ E (encodeFoldable . mapMaybe unwrapPS . Map.elems) zerepochScripts)
    !> Omit nullDats (Key 4 $ E toCBOR dats)
    !> Omit nullRedeemers (Key 5 $ To rdmrs)
  where
    unwrapTS (TimelockScript x) = Just x
    unwrapTS _ = Nothing
    unwrapPS (ZerepochScript x) = Just x
    unwrapPS _ = Nothing
    (zerepochScripts, timelocks) = Map.partition isZerepochScript scripts
  ...
  fromCBOR =
    ...
    where
      ...
      txWitnessField 3 =
        fieldA
          addScripts
          (fmap ZerepochScript <$> listDecode)
      txWitnessField 4 =
        fieldAA
