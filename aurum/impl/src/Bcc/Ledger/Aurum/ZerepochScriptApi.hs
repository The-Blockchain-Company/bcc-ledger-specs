...
module Bcc.Ledger.Aurum.ZerepochScriptApi
  ( -- Figure 8
    getData,
    evalScripts,
    -- Figure 12
...
import Bcc.Ledger.Aurum.Data (getZerepochData)
...
import qualified Bcc.Ledger.Aurum.Scripts as AurumScript (Script (..))
...
import qualified Bcc.Ledger.Aurum.TxBody as Aurum (TxBody (..), TxOut (..), vldt')
import Bcc.Ledger.Aurum.TxInfo
  ( FailureDescription (..),
    ScriptResult (..),
    andResult,
    runPLCScript,
...
import Bcc.Ledger.Aurum.TxWitness (TxWitness (txwitsVKey'), txscripts', unTxDats)
import Bcc.Ledger.BaseTypes (StrictMaybe (..))
import qualified Bcc.Ledger.Core as Core
import Bcc.Ledger.Credential (Credential (ScriptHashObj))
import qualified Bcc.Ledger.Crypto as CC (Crypto)
import Bcc.Ledger.Era (Crypto, Era)
import Bcc.Ledger.Jen.Value (PolicyID (..))
import qualified Bcc.Ledger.Jen.Value as Jen (Value (..))
import Bcc.Ledger.SophieMA.Timelocks (evalTimelock)
...
import Sophie.Spec.Ledger.UTxO (UTxO (..), getScriptHash, scriptCred)
...
collectTwoPhaseScriptInputs ::
  forall era tx.
  ( Era era,
    Core.Script era ~ AurumScript.Script era,
    Core.TxOut era ~ Aurum.TxOut era,
    Core.TxBody era ~ Aurum.TxBody era,
    Core.Value era ~ Jen.Value (Crypto era),
    HasField "datahash" (Core.TxOut era) (StrictMaybe (DataHash (Crypto era))),
  ...
  tx ->
  UTxO era ->
  Either [CollectError (Crypto era)] [(AurumScript.Script era, [Data era], ExUnits, CostModel)]
collectTwoPhaseScriptInputs ei sysS pp tx utxo =
  case Map.lookup ZerepochV1 (getField @"_costmdls" pp) of
    Nothing -> Left [NoCostModel ZerepochV1]
  ...
  where
    ...
    knownToNotBe1Phase (_, sh) =
      case sh `Map.lookup` txscripts' (getField @"wits" tx) of
        Just (AurumScript.ZerepochScript _) -> True
        Just (AurumScript.TimelockScript _) -> False
        Nothing -> True
    redeemer (sp, _) =
...
language :: AurumScript.Script era -> Maybe Language
language (AurumScript.ZerepochScript _) = Just ZerepochV1
language (AurumScript.TimelockScript _) = Nothing
...
evalScripts ::
  forall era tx.
  ( Era era,
    Aurum.TxBody era ~ Core.TxBody era,
    Show (AurumScript.Script era),
    HasField "body" tx (Core.TxBody era),
    HasField "wits" tx (TxWitness era)
  ) =>
  tx ->
  [(AurumScript.Script era, [Data era], ExUnits, CostModel)] ->
  ScriptResult
evalScripts _tx [] = Passes
evalScripts tx ((AurumScript.TimelockScript timelock, _, _, _) : rest) =
  lift (evalTimelock vhks (Aurum.vldt' (getField @"body" tx)) timelock)
...
evalScripts tx ((AurumScript.ZerepochScript pscript, ds, units, cost) : rest) =
  runPLCScript (Proxy @era) cost pscript units (map getZerepochData ds)
    `andResult` evalScripts tx rest
