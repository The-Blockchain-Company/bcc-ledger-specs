...
-- HasTrace instances for AurumLEDGE
{-# OPTIONS_GHC -fno-warn-orphans #-}

-- | Export several sets of property tests for the Aurum Era.
--     Also expert some functions that can be used to debug the Aurum Era property test generators.
module Test.Bcc.Ledger.Aurum.Trials
  ( aurumPropertyTests,
    fastPropertyTests,
    genstuff,
    genAurumTx,
...
import Bcc.Ledger.Aurum.Scripts (ppScript)
...
import Bcc.Ledger.Hashes (EraIndependentData)
import Bcc.Ledger.Pretty (PDoc, PrettyA (..))
...
import Sophie.Spec.Ledger.STS.Delegs (DelegsEnv)
import Sophie.Spec.Ledger.STS.Delpl (DelplEnv, DelplPredicateFailure)
import Sophie.Spec.Ledger.STS.Ledger (LedgerEnv (..))
import Sophie.Spec.Ledger.STS.Utxo (UtxoEnv)
import Sophie.Spec.Ledger.TxBody (DCert, TxIn)
import System.Timeout
import Test.Bcc.Ledger.Aurum.AurumEraGen ()
import qualified Test.Bcc.Ledger.Aurum.PropertyTests as Aurum
import Test.Bcc.Ledger.EraBuffet (TestCrypto)
import Test.Sophie.Spec.Ledger.ConcreteCryptoTypes (Mock)
import Test.Sophie.Spec.Ledger.Generator.Block (genBlock)
import Test.Sophie.Spec.Ledger.Generator.Constants (Constants (..))
import Test.Sophie.Spec.Ledger.Generator.Core (GenEnv (..), KeySpace (..), ScriptSpace (..), hashData)
import Test.Sophie.Spec.Ledger.Generator.EraGen (EraGen (..), MinLEDGER_STS, allScripts, genUtxo0)
...
import Test.Sophie.Spec.Ledger.Generator.Trace.Ledger (genAccountState)
import Test.Sophie.Spec.Ledger.Generator.Utxo (genTx)
import Test.Sophie.Spec.Ledger.PropertyTests
  ( bccPreservationChain,
    collisionFreeComplete,
    delegProperties,
    minimalPropertyTests,
...
--     See genAurumTx and genAurumBlock as examples of its use.
genstuff ::
  ...
  ( GenEnv era ->
    ...
    LedgerState era ->
    Core.PParams era ->
    Sophie.Spec.Ledger.LedgerState.UTxOState era ->
    DPState (Crypto era) ->
    DState (Crypto era) ->
    PState (Crypto era) ->
    Gen b
...
type L = AurumLEDGER A

ap :: Proxy A
ap = Proxy

-- The AurumLEDGER STS combines utxo and delegation rules and allows for generating transactions
...
instance
  ...
  TQC.HasTrace (AurumLEDGER era) (GenEnv era)
  where
  ...
  type BaseEnv (AurumLEDGER era) = Globals
...
genAurumTx :: Gen (Core.Tx A)
genAurumTx = genstuff ap (\genv _cs _nep _ep _ls _pp utxo dp _d _p -> genTx genv ledgerEnv (utxo, dp))

genAurumBlock :: Gen (Block A)
genAurumBlock = genstuff ap (\genv cs _nep _ep _ls _pp _utxo _dp _d _p -> genBlock genv cs)

genSophieTx :: Gen (Core.Tx (SophieEra TestCrypto))
genSophieTx =
  genstuff
    (Proxy @(SophieEra TestCrypto))
    (\genv _cs _nep _ep _ls _pp utxo dp _d _p -> genTx genv ledgerEnv (utxo, dp))

genSophieBlock :: Gen (Block (SophieEra TestCrypto))
genSophieBlock = genstuff (Proxy @(SophieEra TestCrypto)) (\genv cs _nep _ep _ls _pp _utxo _dp _d _p -> genBlock genv cs)
...
-- | The same property tests run in all the other Eras, specialized to the Aurum Era.
aurumPropertyTests :: TestTree
aurumPropertyTests =
  testGroup
    "Aurum property tests"
    [ propertyTests @A @L,
      Aurum.propertyTests
    ]
...
fastPropertyTests =
  testGroup
    "Fast Aurum Property Tests"
    [ testProperty
        "total amount of Bcc is preserved (Chain)"
        (withMaxSuccess 50 (bccPreservationChain @A @L))
    ]
          ...
          case prop of
            ...
            "bcc" ->
              ( testProperty
                  ...
                  (withMaxSuccess count (bccPreservationChain @A @L))
              )
            "deleg" ->
              ( testProperty
  ...
  timeout (seconds * 1000000) $
    defaultMain
      ( localOption
          ...
          (testProperty "preserves BCC" $ bccPreservationChain @A @L)
