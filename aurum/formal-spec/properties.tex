...
\begin{definition}
  ...
  similar to the definition of $\Val$ in the Sophie specification.
...
\begin{property}[General Accounting]
  \label{prop:pov}
  The \emph{general accounting} property in Aurum encompasses two parts
  \begin{itemize}
    ...
    is specified as in the SophieMA POV.
...
\begin{proof}
  ...
  $\fun{collateral}~(\fun{txbody}~tx)$ from the UTxO, and increases the fee pot by the amount of the sum of Bcc in the
  ...
  that, in the case that there are phase-2 scripts, there are no non-Bcc tokens in the UTxOs
  which the collateral inputs reference, so non-Bcc tokens do not get minted, burned, or transferred
  in this case.
\end{proof}
\end{property}
...
\begin{property}[Extended UTxO Validation]
  ...
  \begin{proof}
   ...
   \begin{itemize}
     \item the fee pot, which is increased by the amount of the sum of Bcc in the
...
\begin{proof}
  ...
  transfers the amount of Entropic in the $\fun{txfee}$ field of the
...
\begin{proof}
  By inspecting each transition in this specification, as well as those inherited from the
    Sophie one, we see that a non-trivial update from the UTxO of $s$ to that of $s'$
...
\begin{proof}
    ...
    constructs a list where each entry contains a Zerepoch script
    ...
    \begin{itemize}
      ...
      Therefore, for all Zerepoch script-locked realized inputs of either transaction,
      ...
      \[ \fun{txInfo}~\ZerepochVI~\Utxo(s)~\var{tx} = \fun{txInfo}~\ZerepochVI~\Utxo(u)~\var{tx'}\]
...
\begin{proof}
  ...
  The first argument (of type $\Tx$) is only inspected in the case that the script $sc$ (the first element
    ...
    are checked in phase one of validation (see \ref{fig:rules:utxow-aurum}) by calling $\fun{validateScript}$
...
\begin{enumerate}
\item
  \emph{Commutativity of Translation.} Translate, then apply to Aurum ledger is
