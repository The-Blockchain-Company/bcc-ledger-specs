...
module Cole.Spec.Chain.STS.Block where

import Cole.Spec.Ledger.Core (Hash (Hash), Sig (Sig), Slot, VKey, hash, owner)
import Cole.Spec.Ledger.Delegation
import Cole.Spec.Ledger.UTxO (Tx, TxIn, TxOut, Wit)
import Cole.Spec.Ledger.Update (ProtVer, STag, UProp, Vote)
...
import Test.Shepard (AddShrinks (..), Shepard (..))
import Test.Shepard.TH (deriveAddShrinks, deriveShepard)
...
-- We declare a specific instance here to avoid recursing into bcc-crypto
...
-- epoch boundary blocks will be largely ignored in the Cole-Sophie bridge.
data Block = Block
...
deriveShepard ''Block
deriveShepard ''BlockBody
deriveShepard ''BlockHeader
