...
module Cole.Spec.Chain.STS.Rule.Chain where

import Cole.Spec.Chain.STS.Block
import Cole.Spec.Chain.STS.Rule.BBody
import Cole.Spec.Chain.STS.Rule.Epoch (EPOCH, sEpoch)
import Cole.Spec.Chain.STS.Rule.Pbft
import qualified Cole.Spec.Chain.STS.Rule.SigCnt as SigCntGen
...
import Cole.Spec.Ledger.STS.UTXO (UTxOEnv (UTxOEnv, pps, utxo0), UTxOState)
import Cole.Spec.Ledger.STS.UTXOWS (UTXOWS)
import Cole.Spec.Ledger.UTxO (UTxO, mapUTxOValues)
import Cole.Spec.Ledger.Update hiding (delegationMap)
import qualified Cole.Spec.Ledger.Update as Update
...
instance HasTrace CHAIN where
  envGen chainLength = do
    ...
    where
      ...
      adjustUTxO = mapUTxOValues $ \(Entropic v) ->
        Entropic $ v * fromIntegral GP.c
