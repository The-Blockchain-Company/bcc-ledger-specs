...
\begin{abstract}
  ...
  specification of the blockchain layer of Bcc in the Cole release and in
  a transition to the Sophie release.
  %
...
%
The scope of the document is the Cole release and a transition phase to the
Sophie release of the Bcc blockchain platform.
...
This document is intended to be read in conjunction with \cite{cole_ledger_spec},
...
\begin{figure}[ht]
  \begin{equation*}
    \inference
    { \Gamma \vdash \var{us}
      \trans{\hyperlink{cole_ledger_spec_link}{upireg}}{\var{prop}} \var{us'}
      \\
      \Gamma \vdash \var{us'}
      \trans{\hyperlink{cole_ledger_spec_link}{upivotes}}{\var{votes}} \var{us''}
      &
      \Gamma \vdash \var{us''}
      \trans{\hyperlink{cole_ledger_spec_link}{upiend}}{\var{end}} \var{us'''}
    }
    {
      \Gamma \vdash
      {\var{us}}
...
Bcc's planned evolution can be split into roughly three eras:
\begin{description}
\item[Cole/Shardagnostic] In the Cole/Shardagnostic era, the Shardagnostic (\cite{shardagnostic})
  protocol is used to control who is eligible to issue a block, using a stake
  distribution mediated by heavyweight delegation certificates. The Cole
...
\item[Handover] In the handover era, blocks will be issued according to
  Shardagnostic BFT (\cite{shardagnostic_bft}). The Cole payload will be retained, although
  parts of will be superfluous.
\item[Sophie/Optimum] In the Sophie/Optimum era, blocks will be issued according
  to the Shardagnostic Optimum (\cite{shardagnostic_optimum}) protocol, with stake distribution
...
issued according to Shardagnostic BFT, they will be validated according to a variant
known as Permissive BFT. This is designed such that it will successfully
validate blocks issued both under Shardagnostic and under Shardagnostic BFT (with a high
...
It is also worth noticing that in the Cole era, the number of slots per-epoch
...
\begin{equation*}
  \inference
  {
    \var{e_c} < \sepoch{s}{k}
    &
    \sepoch{s}{k}\vdash \var{us} \trans{\hyperlink{cole_ledger_spec_link}{upiec}}{} \var{us'}
  }
  {
    \var{e_c}
    \vdash
...
protocol parameter is defined in \cite{cole_ledger_spec}.
...
header. Note that Cole-era block payload also has an additional component: the
  ...
  \begin{equation*}
    ...
    { \maxblocksize \mapsto \var{b_{max}} \in \var{pps} && \bsize{b} \leq \var{b_{max}} \\
      ...
      {\left(
          ...
          \end{array}
        \right)}
      \vdash \var{ds} \trans{\hyperlink{cole_ledger_spec_link}{deleg}}{\bcerts{b}} \var{ds'} &
      {\left(
          \begin{array}{l}
            \var{utxo_0} \\
            \var{pps}
        ...
        \right)}
      \vdash \var{utxoSt}
        \trans{\hyperlink{cole_ledger_spec_link}{utxows}}{\butxo{b}} \var{utxoSt'} \\
    }
    {
      \left(
        {\begin{array}{l}
...
\begin{appendices}
  ...
  to prevent a situation in which a chain produced under the initial Shardagnostic
  ...
  the slot leaders for each slot. Given the Bcc chain is still federated, the
