...
module Bcc.Crypto.Signing.SigningKey
  ( SigningKey (..),
    toVerification,
    toCBORXPrv,
    fromCBORXPrv,
...
import Bcc.Binary (Decoder, Encoding, FromCBOR (..), ToCBOR (..))
import Bcc.Crypto.Signing.VerificationKey (VerificationKey (..), shortVerificationKeyHexF)
...
-- Note that there is deliberately no Eq instance. The bcc-crypto library
