module Bcc.Crypto.Signing.Redeem.KeyGen
  ( redeemKeyGen,
    redeemDeterministicKeyGen,
  )
where

import Bcc.Crypto.Signing.Redeem.SigningKey (RedeemSigningKey (..))
import Bcc.Crypto.Signing.Redeem.VerificationKey (RedeemVerificationKey (..))
import Bcc.Prelude
...
--   "Bcc.Crypto.Random" because the OpenSSL generator is probably safer
...
redeemDeterministicKeyGen seed =
  case maybeCryptoError $ Ed25519.secretKey (BA.convert seed :: BA.Bytes) of
    ...
    Nothing ->
      fail "Bcc.Crypto.Signing.Redeem.hs redeemDeterministicKeyGen failed"
