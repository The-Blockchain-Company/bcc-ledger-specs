...
module Test.Cole.Spec.Ledger.AbstractSize.Properties (testTxHasTypeReps) where
...
propSumOfSizesTx txw =
  ...
  where
    ...
    txOutCosts = Map.unions [mkCost @TxOut, mkCost @Addr, mkCost @VKey, mkCost @Entropic]
...
propMultipleOfSizes txw =
  let body_ = body txw
   ...
   in do
        ...
        -- we should account for each TxOut/Addr/Entropic in a TxWits's size
        abstractSize (mkCost @TxOut) txw === length (outputs body_)
        abstractSize (mkCost @Addr) txw === length (outputs body_)
        abstractSize (mkCost @Entropic) txw === length (outputs body_)
        -- the combined cost is the sum of individual costs
        abstractSize (Map.unions [mkCost @TxOut, mkCost @Addr, mkCost @Entropic]) txw
          === abstractSize (mkCost @TxOut) txw
            + abstractSize (mkCost @Addr) txw
            + abstractSize (mkCost @Entropic) txw
