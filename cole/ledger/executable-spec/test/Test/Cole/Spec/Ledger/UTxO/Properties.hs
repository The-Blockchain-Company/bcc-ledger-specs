...
module Test.Cole.Spec.Ledger.UTxO.Properties where

import Cole.Spec.Ledger.Core (Entropic, dom, unEntropic, (∩), (∪), (⋪), (◁))
import Cole.Spec.Ledger.STS.UTXO (UTxOState (UTxOState), pps, reserves, utxo)
...
relevantCasesAreCovered = withTests 400 $
  ...
  where
    ...
    txFeeSurplus :: (Tx -> Entropic) -> (UTxOState, Tx) -> Integer
    txFeeSurplus txMinFee (st, txw) =
      fee - minFee
      where
        tx_ = body txw
        utxo_ = utxo st
        fee = unEntropic $ balance (txins tx_ ◁ utxo_) - balance (txouts tx_)
        minFee = unEntropic $ txMinFee txw
