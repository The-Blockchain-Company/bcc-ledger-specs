...
module Cole.Spec.Ledger.Update
  ( module Cole.Spec.Ledger.Update,
    PredicateFailure (),
  )
where
...
import qualified Cole.Spec.Ledger.Core as Core
...
-- | Protocol parameters.
data PParams = PParams -- TODO: this should be a module of @cole-spec-ledger@.
...
--
--  >>> import qualified Data.Map.Strict as Map
--  >>> import Cole.Spec.Ledger.Update (invertMap)
...
pvCanFollow ::
  ...
  ProtVer ->
  Bool
pvCanFollow (ProtVer mjn sn) (ProtVer mjp sp) =
    (mjp, sp) < (mjn, sn)
    && (inRange (0, 1) (mjn - mjp))
    && ((mjp == mjn) ==> (sp + 1 == sn))
    && ((mjp + 1 == mjn) ==> (sn - 1 == sp))
...
instance HasHash (Maybe Cole.Spec.Ledger.Update.UProp, [Cole.Spec.Ledger.Update.Vote]) where
        ...
        where
          ...
          nextAltVersion (maj, mn) =
            ...
            where 
              ...
              nextVersion (x : _) = ProtVer (maj*x) mn
...
-- disentangle the dependencies. Moving this to @Cole.Spec.Ledger.Update.Generators@ will
        ...
        where
          vk = fromMaybe err $ Bimap.lookup vkg dms
            where
              err =
                error $
                  "Cole.Spec.Ledger.Update.mkVoteForDelegate: "
...
deriveShepard ''ApVer
deriveShepard ''ApName
deriveShepard ''Metadata
deriveShepard ''ProtVer
deriveShepard ''PParams
deriveShepard ''SwVer
deriveShepard ''UpId
deriveShepard ''UProp
deriveShepard ''Vote

instance GeneOps s => Shepard s FactorA where
  tinker gen =
    tinkerRummagedOrConjureOrSave
      ( (\x -> FactorA (x `mod` fromIntegral GP.entropicCap))
          <$$> tinker ((\(FactorA x) -> x) <$> gen)
      )
  conjure =
    saveInBagOfTricks
      =<< ( FactorA . (`mod` fromIntegral GP.entropicCap)
              <$> conjure
          )

instance GeneOps s => Shepard s FactorB where
  tinker gen =
    tinkerRummagedOrConjureOrSave
      ( (\x -> FactorB (x `mod` fromIntegral GP.entropicCap))
          <$$> tinker ((\(FactorB x) -> x) <$> gen)
      )
  conjure =
    saveInBagOfTricks
      =<< ( FactorB . (`mod` fromIntegral GP.entropicCap)
              <$> conjure
          )

instance GeneOps s => Shepard s BkSgnCntT where
  tinker _ =
    pure <$> conjure
  conjure =
...
instance GeneOps s => Shepard s UpAdptThd where
  tinker _ =
    pure <$> conjure
  conjure =
    saveInBagOfTricks =<< do
...
deriveSeedShepard ''ApName
deriveSeedShepard ''ApVer
deriveSeedShepard ''BkSgnCntT
deriveSeedShepard ''FactorA
deriveSeedShepard ''FactorB
deriveSeedShepard ''SwVer
deriveSeedShepard ''PParams
deriveSeedShepard ''ProtVer
deriveSeedShepard ''Metadata
deriveSeedShepard ''UpAdptThd
deriveSeedShepard ''UpId
...
tamperWithUpdateProposal env st uprop = do
  ...
  let failureGenerators =
        [ (1, invalidProtocolVersion),
          ...
          ++ (map (\sg -> (1, sg env st)) shepardGensUPIREG)
  ...
  where
    ...
    invalidProtocolVersion =
      (\mj sn -> uprop {_upPV = ProtVer mj sn})
...
tamperWithVotes env st vs =
  Gen.frequency
    [ (1, go vs),
      (1, Gen.choice (map (\sg -> sg env st) shepardGensUPIVOTES))
    ]
  where
    go [] = do
