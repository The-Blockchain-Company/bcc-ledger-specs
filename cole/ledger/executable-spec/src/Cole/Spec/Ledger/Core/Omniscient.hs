...
module Cole.Spec.Ledger.Core.Omniscient where
...
-- | Sign using a vested verifying key.
--
-- See 'skey' for details about situations where this function is used.
signWithVestedKey :: VKeyVested -> a -> Sig a
signWithVestedKey vka = sign (skey (unVKeyVested vka))
