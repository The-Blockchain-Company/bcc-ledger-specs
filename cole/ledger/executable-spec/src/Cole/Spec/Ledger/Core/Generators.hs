...
module Cole.Spec.Ledger.Core.Generators
  ( vkGen,
    vkgenesisGen,
    vkvestedGen,
    addrGen,
...
import Cole.Spec.Ledger.Core
  ( Addr (Addr),
    ...
    VKeyVested (VKeyVested),
  )
import Cole.Spec.Ledger.GlobalParams (slotsPerEpochToK)
...
vkvestedGen :: Gen VKeyVested
vkvestedGen = VKeyVested <$> vkGen
