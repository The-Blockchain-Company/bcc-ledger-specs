...
module Cole.Spec.Ledger.UTxO.Generators where

import Cole.Spec.Ledger.Core hiding (Range, range)
...
genInitialTxOuts =
  Gen.filter (not . null)
    . genTraverseSubsequence (\a -> TxOut a <$> genEntropic)

genEntropic :: Gen Entropic
genEntropic =
  Entropic . fromIntegral <$> Gen.word32 (Range.linearFrom mid mn mx)
  where
    mn = 1
    mx = 10000
...
genTxFromUTxO addrs txfee (UTxO utxo) = do
  ...
  where
    genTxBody =
      uncurry TxBody
        <$> Gen.filter
          ...
          ( genInputOutput
              (M.keys utxo)
              (maybe 0 (unEntropic . value) . flip M.lookup utxo)
              (fmap (. Entropic) $ TxOut <$> Gen.element addrs)
              (unEntropic . value)
              (\f out -> out {value = Entropic . f . unEntropic $ value out})
          )
    ...
    subtractFees =
      fmap subtractFees'
        -- In Cole, we must disallow empty outputs in transactions in order to
        -- maintain compatability with the `bcc-sl` implementation.
        ...
        -- least 1 'Entropic' in the outputs.
