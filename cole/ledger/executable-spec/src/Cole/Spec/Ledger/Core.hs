...
module Cole.Spec.Ledger.Core where
...
-- | An Vested key is a specialisation of a generic VKey.
newtype VKeyVested = VKeyVested {unVKeyVested :: VKey}
  deriving stock (Show, Generic, Data, Typeable)
  deriving newtype (Eq, Ord, Hashable, HasHash, FromCBOR, ToCBOR, NoThunks)
  deriving anyclass (HasTypeReps)

instance HasOwner VKeyVested where
  owner (VKeyVested vk) = owner vk

mkVKeyVested :: Natural -> VKeyVested
mkVKeyVested = VKeyVested . VKey . Owner

-- | Make a set of vested keys. The vested keys are continuously numbered from 0 to the given
-- number of vested keys minus 1.
mkVkVestedSet ::
  -- | Number of vested keys
  Word8 ->
  Set VKeyVested
mkVkVestedSet ngk = Set.fromAscList $ mkVKeyVested <$> [0 .. (fromIntegral ngk - 1)]
...
-- | Constant amount of Entropic in the system.
entropicCap :: Entropic
entropicCap = Entropic $ 45 * fromIntegral ((10 :: Int64) ^ (15 :: Int64))
...
--
-- Unicode: 2286
(⊆) :: (Foldable f, Foldable s, Ord a) => f a -> s a -> Bool
...
instance GeneOps s => Shepard s Hash where
  tinker gen =
    tinkerRummagedOrConjureOrSave
      ( (Hash . Just . (`mod` 30))
...
instance GeneOps s => Shepard s Entropic where
  tinker gen =
    tinkerRummagedOrConjureOrSave
      ( (\x -> (Entropic x) `mod` entropicCap)
          <$$> tinker ((\(Entropic x) -> x) <$> gen)
      )
  conjure = saveInBagOfTricks =<< ((`mod` entropicCap) . Entropic <$> conjure)
...
deriveAddShrinks ''VKeyVested

--------------------------------------------------------------------------------
-- SeedShepard instances
--------------------------------------------------------------------------------

deriveSeedShepard ''Addr
deriveSeedShepard ''BlockCount
deriveSeedShepard ''Epoch
...
deriveSeedShepard ''VKeyGenesis
deriveSeedShepard ''VKeyVested
