...
module Bcc.Chain.Common.Merkle
  ( -- * MerkleRoot
    MerkleRoot (..),
...
-- Bcc.Prelude has its own Rube Goldberg variant of 'Foldable' which we do not
-- want. It would be great if we could write
--   import           Bcc.Prelude hiding (toList, foldMap)
...
import Bcc.Crypto (Hash, hashDecoded, hashRaw, hashToBytes)
import Bcc.Prelude
