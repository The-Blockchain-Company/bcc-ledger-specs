...
module Bcc.Chain.Common.TxFeePolicy
  ( TxFeePolicy (..),
  )
where
...
import Bcc.Chain.Common.TxSizeLinear (TxSizeLinear (..))
import Bcc.Prelude
...
instance Monad m => ToJSON m TxFeePolicy where
  ...
  toJSON (TxFeePolicyTxSizeLinear (TxSizeLinear summand multiplier)) =
    mkObject
      [ ("summand", toJSON $ 1e9 * entropicToInteger summand),
...
instance MonadError SchemaError m => FromJSON m TxFeePolicy where
  ...
  fromJSON obj = do
    summand <-
      wrapEntropicError . mkEntropic . (`div` 1e9)
        =<< fromJSField
          obj
          "summand"
    multiplier <-
    ...
    where
      wrapEntropicError :: Either EntropicError Entropic -> m Entropic
      wrapEntropicError =
        either (expected "Entropic" . Just . formatToString build) pure
