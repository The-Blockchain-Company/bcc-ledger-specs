...
module Bcc.Chain.Common.Address
  ( Address (..),
    Address' (..),
...
import Bcc.Chain.Common.AddrAttributes
  ( AddrAttributes (..),
    HDAddressPayload,
  )
import Bcc.Chain.Common.AddrSpendingData
  ( AddrSpendingData (..),
    AddrType (..),
    addrSpendingDataToType,
  )
import Bcc.Chain.Common.AddressHash (AddressHash, addressHash)
import Bcc.Chain.Common.Attributes (Attributes (..), mkAttributes)
import Bcc.Chain.Common.CBOR
  ( decodeCrcProtected,
    encodeCrcProtected,
    encodedCrcProtectedSizeExpr,
  )
import Bcc.Chain.Common.NetworkMagic (NetworkMagic (..))
import Bcc.Crypto.Hashing (hashHexF)
...
-- | 'Address' is where you can send Entropic
