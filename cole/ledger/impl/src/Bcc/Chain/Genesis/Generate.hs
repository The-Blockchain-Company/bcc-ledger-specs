...
module Bcc.Chain.Genesis.Generate
  ( GeneratedSecrets (..),
    gsSigningKeys,
    gsSigningKeysPoor,
    PoorSecret (..),
...
import Bcc.Chain.Common.NetworkMagic (makeNetworkMagic)
import qualified Bcc.Chain.Delegation.Certificate as Delegation
import Bcc.Chain.Genesis.AvvmBalances (GenesisAvvmBalances (..))
import Bcc.Chain.Genesis.Config (Config (..))
import Bcc.Chain.Genesis.Data (GenesisData (..))
import Bcc.Chain.Genesis.Delegation
  ( GenesisDelegation (..),
    GenesisDelegationError,
    mkGenesisDelegation,
  )
import Bcc.Chain.Genesis.Hash (GenesisHash (..))
import Bcc.Chain.Genesis.Initializer
  ( FakeAvvmOptions (..),
    GenesisInitializer (..),
...
import Bcc.Chain.Genesis.Spec (GenesisSpec (..))
import Bcc.Chain.UTxO.UTxOConfiguration (defaultUTxOConfiguration)
import Bcc.Crypto as Crypto
  ( RedeemSigningKey,
    SigningKey,
    getProtocolMagicId,
...
data GenesisDataGenerationError
  ...
  | GenesisDataGenerationDistributionMismatch Entropic Entropic
  | GenesisDataGenerationEntropicError EntropicError
  ...
  build = \case
    ...
    GenesisDataGenerationDistributionMismatch testBalance totalBalance ->
      bprint
        ...
        testBalance
        totalBalance
    GenesisDataGenerationEntropicError entropicErr ->
      bprint
        ( "GenesisDataGenerationEntropicError: "
            . build
        )
        entropicErr
    GenesisDataGenerationPassPhraseMismatch ->
...
generateGenesisDataWithEntropy startTime genesisSpec = do
  ...
  -- Real AVVM Balances
  let applyAvvmBalanceFactor :: Map k Entropic -> Map k Entropic
      applyAvvmBalanceFactor =
        map (flip scaleEntropicRational (giAvvmBalanceFactor gi))
  ...
  totalFakeAvvmBalance <-
    scaleEntropic (faoOneBalance fao) (faoCount fao)
      `wrapError` GenesisDataGenerationEntropicError

  -- Compute total balance to generate
  avvmSum <-
    sumEntropic (unGenesisAvvmBalances realAvvmMultiplied)
      `wrapError` GenesisDataGenerationEntropicError
  maxTnBalance <-
    subEntropic maxBound avvmSum `wrapError` GenesisDataGenerationEntropicError
  ...
  nonAvvmBalance <-
    subEntropic tnBalance totalFakeAvvmBalance
      `wrapError` GenesisDataGenerationEntropicError
...
genTestnetDistribution tbo testBalance = do
  (richBalances, poorBalances, totalBalance) <-
    (`wrapError` GenesisDataGenerationEntropicError) $ do
      richmanBalance <- divEntropic desiredRichBalance tboRichmen

      richmanBalanceExtra <- modEntropic desiredRichBalance tboRichmen
      ...
      richmanBalance' <-
        if tboRichmen == 0
          then pure $ mkKnownEntropic @0
            ...
            addEntropic
              richmanBalance
              ( if richmanBalanceExtra > mkKnownEntropic @0
                  then mkKnownEntropic @1
                  else mkKnownEntropic @0
              )

      totalRichBalance <- scaleEntropic richmanBalance' tboRichmen

      desiredPoorsBalance <- subEntropic testBalance totalRichBalance

      poorBalance <-
        if tboPoors == 0
          then pure $ mkKnownEntropic @0
          else divEntropic desiredPoorsBalance tboPoors

      totalPoorBalance <- scaleEntropic poorBalance tboPoors

      totalBalance <- addEntropic totalRichBalance totalPoorBalance
  ...
  where
    ...
    desiredRichBalance = scaleEntropicRational testBalance (tboRichmenShare tbo)
