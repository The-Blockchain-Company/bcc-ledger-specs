...
module Bcc.Chain.Delegation.Validation.Interface
...
import Bcc.Chain.Common (BlockCount (..), KeyHash, hashKey)
import qualified Bcc.Chain.Delegation as Delegation
import Bcc.Chain.Delegation.Certificate (ACertificate, Certificate)
import qualified Bcc.Chain.Delegation.Validation.Activation as Activation
import qualified Bcc.Chain.Delegation.Validation.Scheduling as Scheduling
import Bcc.Chain.Genesis (GenesisDelegation (..))
import Bcc.Chain.Slotting
  ( EpochNumber,
    SlotNumber (..),
  )
import Bcc.Crypto (ProtocolMagicId, VerificationKey)
...
--
--   This corresponds to the `DELEG` rule from the Cole ledger specification
updateDelegation ::
  MonadError Scheduling.Error m =>
  Environment ->
  State ->
