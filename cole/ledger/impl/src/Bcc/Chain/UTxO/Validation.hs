...
module Bcc.Chain.UTxO.Validation
  ( validateTx,
    validateTxAux,
    updateUTxO,
    updateUTxOTxWitness,
...
import Bcc.Chain.UTxO.Compact (CompactTxOut (..), toCompactTxIn)
import Bcc.Chain.UTxO.Tx (Tx (..), TxIn, TxOut (..))
import Bcc.Chain.UTxO.TxAux (ATxAux (..), aTaTx, taWitness)
import Bcc.Chain.UTxO.TxWitness
  ( TxInWitness (..),
    TxSigData (..),
    recoverSigData,
  )
import Bcc.Chain.UTxO.UTxO
  ( UTxO,
    UTxOError,
    balance,
    isRedeemUTxO,
...
data TxValidationError
  = TxValidationEntropicError Text EntropicError
  | TxValidationFeeTooSmall Tx Entropic Entropic
...
instance ToCBOR TxValidationError where
  toCBOR = \case
    TxValidationEntropicError text isAacError ->
      encodeListLen 3
        <> toCBOR @Word8 0
        <> toCBOR text
        <> toCBOR isAacError
    TxValidationFeeTooSmall tx entropic1 entropic2 ->
      encodeListLen 4
        <> toCBOR @Word8 1
        <> toCBOR tx
        <> toCBOR entropic1
...
instance FromCBOR TxValidationError where
  fromCBOR = do
    ...
    tag <- decodeWord8
    case tag of
      0 -> checkSize 3 >> TxValidationEntropicError <$> fromCBOR <*> fromCBOR
...
validateTxAux env utxo (ATxAux (Annotated tx _) _ txBytes) = do
  ...
  where
    ...
    calculateMinimumFee ::
      MonadError TxValidationError m => TxFeePolicy -> m Entropic
    calculateMinimumFee = \case
      TxFeePolicyTxSizeLinear txSizeLinear ->
        calculateTxSizeLinear txSizeLinear txSize
          `wrapError` TxValidationEntropicError "Minimum Fee"
