...
import Bcc.Chain.UTxO.Tx (Tx (..), TxId, TxIn (..), TxOut (..))
import Bcc.Crypto (serializeCborHash)
import Bcc.Prelude hiding (concat, empty, toList)
import Data.Coerce
...
fromBalances :: [(Address, Entropic)] -> UTxO
fromBalances =
...
balance :: UTxO -> Either EntropicError Entropic
balance = sumEntropic . fmap compactTxOutValue . M.elems . unUTxO
  where
    compactTxOutValue :: CompactTxOut -> Entropic
