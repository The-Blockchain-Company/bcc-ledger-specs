...
import Bcc.Chain.Common.Compact
  ( CompactAddress,
    fromCompactAddress,
    toCompactAddress,
  )
import Bcc.Chain.Common.Entropic (Entropic)
import Bcc.Chain.UTxO.Tx (TxId, TxIn (..), TxOut (..))
import Bcc.Crypto.Hashing (hashToBytes, unsafeHashFromBytes)
import Bcc.Prelude
...
instance HeapWords CompactTxOut where
  heapWords (CompactTxOut compactAddr _) =
    ...
    -- > newtype Entropic = Entropic { getEntropic :: Word64 }
    --
    -- so @CompactTxOut {-# UNPACK #-} !CompactAddress {-# UNPACK #-} !Entropic@
    -- requires:
    --
    -- - 1 word for the 'CompactTxOut' object header
    -- - 1 word for the pointer to the byte array object
    -- - 1 word (on a 64-bit arch) for the unpacked 'Word64' ('Entropic')
...
instance ToCBOR CompactTxOut where
  toCBOR (CompactTxOut compactAddr entropic) =
    encodeListLen 2
      <> toCBOR compactAddr
      <> toCBOR entropic

toCompactTxOut :: TxOut -> CompactTxOut
toCompactTxOut (TxOut addr entropic) =
  CompactTxOut (toCompactAddress addr) entropic

fromCompactTxOut :: CompactTxOut -> TxOut
fromCompactTxOut (CompactTxOut compactAddr entropic) =
  TxOut (fromCompactAddress compactAddr) entropic
