...
-- TODO `contramap` should be in `Bcc.Prelude`
...
import Bcc.Chain.Block.Boundary
  ( dropBoundaryBody,
...
import Bcc.Chain.Block.Proof (Proof (..))
import Bcc.Chain.Common (ChainDifficulty (..), dropEmptyAttributes)
...
import Bcc.Chain.UTxO.TxPayload (ATxPayload)
import qualified Bcc.Chain.Update.Payload as Update
import Bcc.Chain.Update.ProtocolVersion (ProtocolVersion)
import Bcc.Chain.Update.SoftwareVersion (SoftwareVersion)
import Bcc.Crypto (ProtocolMagicId, SigningKey, VerificationKey)
import Bcc.Prelude
...
-- | Decode a 'Block' accounting for deprecated epoch boundary blocks
--
--   Previous versions of Bcc had an explicit boundary block between epochs.
