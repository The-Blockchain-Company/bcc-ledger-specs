...
module Bcc.Chain.Block.Validation
...
import Bcc.Chain.Block.Body (ABody (..))
import Bcc.Chain.Block.Header
  ( ABoundaryHeader (..),
    AHeader (..),
    BlockSignature,
    HeaderHash,
...
import Bcc.Chain.Block.Proof (Proof (..), ProofValidationError (..))
...
import qualified Bcc.Chain.Delegation.Validation.Interface as DI
import qualified Bcc.Chain.Delegation.Validation.Scheduling as Scheduling
import Bcc.Chain.Epoch.File (ParseError, mainnetEpochSlots)
...
import Bcc.Chain.UTxO (ATxPayload (..), UTxO (..), genesisUtxo, recoverTxProof)
import Bcc.Chain.UTxO.UTxOConfiguration (UTxOConfiguration)
import qualified Bcc.Chain.UTxO.Validation as UTxO
import qualified Bcc.Chain.Update as Update
import Bcc.Chain.Update.Validation.Endorsement (Endorsement (..))
...
-- | This is an implementation of the headerIsValid function from the Cole
--   chain specification
headerIsValid ::
  (MonadError ChainValidationError m, MonadReader ValidationMode m) =>
  UPI.State ->
...
--   rules from the Cole chain specification.
epochTransition ::
  EpochEnvironment ->
  UPI.State ->
  SlotNumber ->
