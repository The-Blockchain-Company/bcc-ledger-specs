...
module Bcc.Chain.Update.SoftforkRule
  ( SoftforkRule (..),
  )
where

import Bcc.Binary (FromCBOR (..), ToCBOR (..), encodeListLen, enforceSize)
import Bcc.Chain.Common (EntropicPortion)
import Bcc.Prelude
