...
module Bcc.Chain.Update.ProtocolParameters
  ( ProtocolParameters (..),
    upAdptThd,
    isBootstrapEraPP,
  )
...
import Bcc.Chain.Slotting (EpochNumber, SlotNumber (..), isBootstrapEra)
import Bcc.Chain.Update.SoftforkRule
import Bcc.Prelude
...
data ProtocolParameters = ProtocolParameters
  { ppScriptVersion :: !Word16,
    ...
    ppMpcThd :: !EntropicPortion,
    ppHeavyDelThd :: !EntropicPortion,
    ppUpdateVoteThd :: !EntropicPortion,
    ppUpdateProposalThd :: !EntropicPortion,
    ...
    -- Cole-Sophie bridge we do not have negative votes, and we aim at
...
-- | In Cole we do not have a @upAdptThd@ protocol parameter, so we have to
--   use the existing ones.
--
--   @entropicPortionToRational . srMinThd . ppSoftforkRule@ will give us the
...
upAdptThd numGenKeys pps =
  ...
  where
    stakeRatio = entropicPortionToRational . srMinThd . ppSoftforkRule $ pps
