...
module Bcc.Chain.Update.Validation.Interface
  ( -- * Environment
    Environment (..),
...
import Bcc.Chain.Common.KeyHash (KeyHash)
...
import Bcc.Chain.Update.Proposal (AProposal, UpId, recoverUpId)
...
import Bcc.Chain.Update.SoftwareVersion
  ( svAppName,
    svNumber,
  )
...
import Bcc.Chain.Update.Validation.Interface.ProtocolVersionBump
  ( tryBumpVersion,
  )
import qualified Bcc.Chain.Update.Validation.Interface.ProtocolVersionBump as PVBump
...
data Environment = Environment
  { protocolMagic :: !(Annotated ProtocolMagicId ByteString),
    ...
    -- @Bcc.Chain.Update.Validation.Endorsement.Environment@.
    numGenKeys :: !Word8,
...
initialState config =
  State
    { currentEpoch = 0,
      adoptedProtocolVersion = ProtocolVersion 0 0,
...
--
--   This corresponds to the @BUPI@ rule in the Cole chain specification.
registerUpdate ::
...
--
-- This corresponds to the @UPIREG@ rule in the Cole ledger specification.
registerProposal ::
  MonadError Error m =>
  Environment ->
  State ->
...
--
-- This corresponds to the @UPIVOTES@ rule in the Cole ledger
-- specification.
registerVotes ::
  MonadError Error m =>
  Environment ->
  ...
  pure $
    ...
    -- argument overwrite the values in the second.
      { appVersions = M.union appVersions' appVersions,
        -- TODO: consider using the `Relation` instances from `bcc-ledger-specs` (see `Ledger.Core`)
        registeredSoftwareUpdateProposals =
          M.withoutKeys
            registeredSoftwareUpdateProposals
            (M.keysSet confirmedProposals)
...
--
-- This corresponds to the @UPIVOTE@ rule in the Cole ledger
registerVote ::
  MonadError Error m =>
  Environment ->
  State ->
...
--
-- This corresponds to the @UPIEND@ rule in the Cole ledger
-- specification.
registerEndorsement ::
  MonadError Error m =>
  Environment ->
...
--
-- This corresponds to the @UPIEC@ rules in the Cole ledger specification.
registerEpoch ::
  Environment ->
  State ->
