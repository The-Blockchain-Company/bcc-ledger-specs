...
--
--   This is an implementation of the rules defined in the Cole ledger
--   specification
module Bcc.Chain.Update.Validation.Registration
  ( Error (..),
    Environment (..),
    State (..),
    ApplicationVersion (..),
...
import Bcc.Chain.Slotting (SlotNumber (SlotNumber))
import Bcc.Chain.Update.ApplicationName (ApplicationName)
import Bcc.Chain.Update.InstallerHash (InstallerHash)
import Bcc.Chain.Update.Proposal
  ( AProposal (..),
    ProposalBody (..),
    UpId,
...
import qualified Bcc.Chain.Update.ProtocolParametersUpdate as PPU
import Bcc.Chain.Update.ProtocolVersion (ProtocolVersion (ProtocolVersion))
...
import Bcc.Chain.Update.SystemTag (SystemTag, SystemTagError, checkSystemTag)
import Bcc.Crypto
  ( ProtocolMagicId (..),
...
registerProposalComponents env rs proposal = do
  ...
  where
    ...
    -- according to the Cole specification. However in the bcc-sl code
    -- they are accepted onto the chain but without any state change.
    --
    -- We cannot follow the legacy bcc-sl interpretation of accepting null
    -- update onto the chain with no effect because it opens the door to DoS
    -- attacks by replaying null update proposals.
    --
    -- For further details see:
    --
    -- https://github.com/The-Blockchain-Company/bcc-ledger/issues/759
    -- https://github.com/The-Blockchain-Company/bcc-ledger/pull/766
    --
    ...
    -- bcc-sl codebase. Therefore we allow for these specific excemptions on
