...
module Test.Bcc.Chain.Elaboration.Block
  ( abEnvToCfg,
    elaborate,
    elaborateBS,
    rcDCert,
...
import Cole.Spec.Chain.STS.Rule.Chain (CHAIN, disL)
import qualified Cole.Spec.Chain.STS.Rule.Epoch as Abstract
import qualified Cole.Spec.Ledger.Core as Abstract
...
import qualified Cole.Spec.Ledger.UTxO as Abstract
import qualified Cole.Spec.Ledger.Update as Abstract.Update
import qualified Bcc.Binary as Binary
import qualified Bcc.Chain.Block as Concrete
...
import qualified Bcc.Chain.Slotting as Slotting
import qualified Bcc.Chain.Ssc as Ssc
...
import Bcc.Crypto.ProtocolMagic (AProtocolMagic (..))
import Bcc.Prelude hiding (to)
...
import Test.Bcc.Chain.Elaboration.Keys
  ( elaborateVKeyGenesis,
    vKeyPair,
    vKeyToSKey,
  )
import Test.Bcc.Chain.Elaboration.Update
  ( elaboratePParams,
...
import Test.Bcc.Chain.UTxO.Model (elaborateTxWitnesses)
  ...
  where
    ...
    bh0 =
      Concrete.mkHeaderExplicit
        ...
        -- TODO: the Cole spec needs to incorporate a software version in the blocks
...
abEnvToCfg (_currentSlot, _genesisUtxo, allowedDelegators, protocolParams, stableAfter) =
  ...
  where
    ...
    -- https://github.com/The-Blockchain-Company/bcc-ledger/issues/332 gets
    -- implemented.
