...
module Test.Bcc.Chain.UTxO.ValidationMode
  ( tests,
  )
where

import qualified Cole.Spec.Ledger.Core as Abstract
import qualified Cole.Spec.Ledger.Core.Generators as Abstract
import qualified Cole.Spec.Ledger.UTxO as Abstract
import qualified Cole.Spec.Ledger.UTxO.Generators as Abstract
import qualified Cole.Spec.Ledger.Update as Abstract
import qualified Cole.Spec.Ledger.Update.Generators as Abstract
import Bcc.Binary (Annotated (..))
import Bcc.Chain.Block (BlockValidationMode (..))
...
import Bcc.Crypto (getProtocolMagicId)
import Bcc.Prelude
...
import Test.Bcc.Chain.Elaboration.UTxO (elaborateTxBS)
import Test.Bcc.Chain.Elaboration.Update (elaboratePParams)
...
-- | Elaborate an 'Abstract.Tx', calculate the 'Concrete.Entropic' fee, then
-- convert back to an 'Abstract.Entropic'.
...
abstractTxFee txIdMap tfp aTx = do
  ...
  -- Add an extra entropic to the fee to compensate for rounding errors
  Abstract.Entropic (1 + entropicToInteger cEntropic)
