module Test.Bcc.Chain.UTxO.Gen
  ( genCompactTxId,
    genCompactTxIn,
    genCompactTxOut,
    genVKWitness,
...
import Bcc.Chain.Common (makeNetworkMagic, mkAttributes)
...
genTxOut :: Gen TxOut
genTxOut = TxOut <$> genAddress <*> genEntropic
...
genTxValidationError = do
  ...
  Gen.choice
    [ TxValidationEntropicError
        <$> Gen.text (Range.constant 0 1000) Gen.alphaNum
        <*> genEntropicError,
      TxValidationFeeTooSmall <$> genTx <*> genEntropic <*> genEntropic,
