...
module Test.Bcc.Chain.Genesis.Example
...
import Bcc.Chain.Delegation (unsafeCertificate)
...
import Test.Bcc.Chain.Update.Example (exampleProtocolParameters)
import Test.Bcc.Crypto.CBOR (getBytes)
import Test.Bcc.Crypto.Example (exampleProtocolMagicId0)
...
exampleGenesisAvvmBalances =
  GenesisAvvmBalances $
    M.fromList
      [ (exampleCompactRVK' (0, 32), mkKnownEntropic @36524597913081152),
        (exampleCompactRVK' (32, 32), mkKnownEntropic @37343863242999412)
      ]
  where
    ...
    { giTestBalance =
        TestnetBalanceOptions
          { tboPoors = 2448641325904532856,
            ...
            tboTotalBalance = mkKnownEntropic @10953275486128625,
            tboRichmenShare = 0.366832547637728 :: Rational
          },
      giFakeAvvmBalance =
        FakeAvvmOptions
          { faoCount = 17853231730478779264,
            faoOneBalance = mkKnownEntropic @15087947214890024
          },
...
exampleGenesisNonAvvmBalances0 =
  ...
  where
    coin = mkKnownEntropic @36524597913081152
    coin1 = mkKnownEntropic @37343863242999412
