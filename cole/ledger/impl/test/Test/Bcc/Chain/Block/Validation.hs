...
module Test.Bcc.Chain.Block.Validation
  ( tests,
  )
where
...
import Bcc.Chain.Epoch.File (ParseError, parseEpochFilesWithBoundary)
import Bcc.Chain.Genesis as Genesis (Config (..), configEpochSlots)
import Bcc.Chain.Slotting (SlotNumber)
import Bcc.Chain.ValidationMode (fromBlockValidationMode)
import Bcc.Prelude
...
import Test.Bcc.Mirror (mainnetEpochFiles)
import Test.Options
  ( ShouldAssertNF (..),
    TSGroup,
    TSProperty,
...
ts_prop_mainnetEpochsValid shouldAssertNF scenario = withTests 1 . property $ do
  menv <- liftIO $ lookupEnv "BCC_MAINNET_MIRROR"
  assert $ isJust menv
