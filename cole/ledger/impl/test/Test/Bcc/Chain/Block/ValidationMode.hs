...
module Test.Bcc.Chain.Block.ValidationMode
  ( tests,
  )
where
...
import qualified Cole.Spec.Chain.STS.Rule.Chain as Abstract
...
import Cole.Spec.Ledger.STS.UTXO (UTxOEnv (..), UTxOState (..))
import Cole.Spec.Ledger.STS.UTXOWS (UTXOWS)
...
import Bcc.Chain.Delegation as Delegation
import Bcc.Chain.UTxO (TxProof)
...
createInitialUTxOState utxoEnv =
  UTxOState {utxo = utxo0, reserves = entropicCap - Abstract.balance utxo0}
  where
    UTxOEnv
      { utxo0
      } = utxoEnv
