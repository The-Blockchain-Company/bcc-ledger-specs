...
module Test.Bcc.Chain.Block.Model
  ( tests,
    elaborateAndUpdate,
...
import Cole.Spec.Chain.STS.Block (BlockStats (..))
import qualified Cole.Spec.Chain.STS.Block as Abstract
import Cole.Spec.Chain.STS.Rule.BBody
...
import Cole.Spec.Chain.STS.Rule.Epoch (sEpoch)
import qualified Cole.Spec.Ledger.Core as AbstractCore
...
import qualified Cole.Spec.Ledger.Delegation.Test as Delegation.Test
import Cole.Spec.Ledger.STS.UTXO (UTxOEnv (..))
import Cole.Spec.Ledger.STS.UTXOW (coverUtxoFailure, tamperedTxList)
...
import qualified Cole.Spec.Ledger.Update.Test as Update.Test
...
import Bcc.Chain.Common (unBlockCount)
...
import Bcc.Chain.Update.Validation.Interface (adoptedProtocolParameters)
import Bcc.Chain.ValidationMode
  ( ValidationMode,
    fromBlockValidationMode,
  )
import Bcc.Prelude hiding (State, state, trace)
...
import Test.Bcc.Chain.Elaboration.Keys (elaborateVKeyGenesisHash)
import Test.Bcc.Chain.UTxO.Model (elaborateInitialUTxO)
      ...
      where
        ...
        invalidDelegationGen env@(sn, _, allowedDelegators, _, k) st =
          ...
          where
            ...
            -- DCerts and shepard-tweaked ones.
...
--
-- TODO: this should be in `bcc-ledger-specs`.
mkUpiEnv ::
  Abstract.Block ->
  Environment CHAIN ->
  State CHAIN ->
...
--
-- TODO: put this in `bcc-ledger-specs`.
mkUpiSt ::
  State CHAIN ->
  UPIState
