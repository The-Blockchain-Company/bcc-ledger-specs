...
module Test.Bcc.Chain.Common.CBOR
  ( tests,
  )
where
...
import Bcc.Prelude hiding (check)
import qualified Data.Map as M
import Hedgehog (Gen, Property, cover, forAll, property, (===))
import qualified Hedgehog as H
import Test.Bcc.Binary.Helpers (SizeTestConfig (..), scfg, sizeTest)
import Test.Bcc.Binary.Helpers.GoldenRoundTrip
  ( goldenTestCBOR,
    roundTripsCBORBuildable,
    roundTripsCBORShow,
  )
import Test.Bcc.Chain.Common.Example
...
import Test.Bcc.Crypto.Gen (genHashRaw)
...
-- Entropic
--------------------------------------------------------------------------------
golden_Entropic :: Property
golden_Entropic = goldenTestCBOR c "test/golden/cbor/common/Entropic"
  where
    c = mkKnownEntropic @9732

ts_roundTripEntropicCBOR :: TSProperty
ts_roundTripEntropicCBOR = eachOfTS 1000 genEntropic roundTripsCBORBuildable

--------------------------------------------------------------------------------
-- EntropicError
--------------------------------------------------------------------------------
ts_roundTripEntropicErrorCBOR :: TSProperty
ts_roundTripEntropicErrorCBOR =
  eachOfTS 1000 genEntropicError roundTripsCBORBuildable

--------------------------------------------------------------------------------
-- EntropicPortion
--------------------------------------------------------------------------------
golden_EntropicPortion :: Property
golden_EntropicPortion =
  goldenTestCBOR c "test/golden/cbor/common/EntropicPortion"
  where
    c = rationalToEntropicPortion 9702e-15

ts_roundTripEntropicPortionCBOR :: TSProperty
ts_roundTripEntropicPortionCBOR =
  eachOfTS 1000 genEntropicPortion roundTripsCBORBuildable
