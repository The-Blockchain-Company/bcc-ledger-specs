...
# The project sources
{ bcc-ledger-specs ? { outPath = ./.; rev = "abcdef"; }

# Function arguments to pass to the project
, projectArgs ? {
    config = { allowUnfree = false; inHydra = true; };
    gitrev = bcc-ledger-specs.rev;
  }
...
with (import pkgs.tbcoNix.release-lib) {
  ...
  packageSet = import bcc-ledger-specs;
  gitrev = bcc-ledger-specs.rev;
};
...
let
  ...
  // {
    ...
    coleLedgerSpec = project.specs.cole-ledger;
    coleChainSpec = project.specs.cole-chain;
    semanticsSpec = project.specs.small-step-semantics;
    sophieLedgerSpec = project.specs.sophie-ledger;
