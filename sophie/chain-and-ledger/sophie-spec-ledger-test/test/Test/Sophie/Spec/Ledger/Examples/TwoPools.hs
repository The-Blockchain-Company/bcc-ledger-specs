...
-- |
-- Module      : Test.Sophie.Spec.Ledger.Examples.TwoPools
...
import Bcc.Ledger.Credential (Credential, Ptr (..))
import qualified Bcc.Ledger.Crypto as CryptoClass
import Bcc.Ledger.Era (Crypto (..))
import Bcc.Ledger.Keys (KeyRole (..), asWitness, coerceKeyRole)
...
import Test.Sophie.Spec.Ledger.ConcreteCryptoTypes (C, C_Crypto, ExMock)
    ...
    [ testCase "create non-aggregated pulser" $ testCHAINExample twoPools9,
      testCase "non-aggregated pulser is correct" $
        ( (Complete (rewardUpdateEx9 @C ppEx rsEx9Agg))
            @?= (runSophieBase . runProvM . completeStep $ pulserEx9 @C ppEx)
        ),
      testCase "aggregated pulser is correct" $
        ( (Complete (rewardUpdateEx9 @C ppProtVer3 rsEx9Agg))
            @?= (runSophieBase . runProvM . completeStep $ pulserEx9 @C ppProtVer3)
        ),
