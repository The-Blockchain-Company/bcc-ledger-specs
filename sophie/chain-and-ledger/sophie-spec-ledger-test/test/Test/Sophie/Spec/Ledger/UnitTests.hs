...
module Test.Sophie.Spec.Ledger.UnitTests (unitTests) where

import Bcc.Binary (serialize')
import Bcc.Crypto.DSIGN.Class (SignKeyDSIGN, VerKeyDSIGN)
...
import Bcc.Ledger.BaseTypes hiding ((==>))
...
import Bcc.Protocol.TOptimum.BHeader (checkLeaderValue)
...
import Sophie.Spec.Ledger.Delegation.Certificates (pattern RegPool)
...
import Sophie.Spec.Ledger.STS.Utxo (UtxoPredicateFailure (..))
...
import Sophie.Spec.Ledger.UTxO (makeWitnessVKey, makeWitnessesVKey)
...
import Test.Sophie.Spec.Ledger.Fees (sizeTests)
...
-- | Test @checkLeaderVal@ in 'Sophie.Spec.Ledger.BlockChain'
testCheckLeaderVal ::
  forall v.
  (v ~ VRF C_Crypto) =>
  -- (v ~ CLVVRF) =>
...
testLEDGER initSt tx env (Right expectedSt) = do
  checkTrace @(LEDGER C) runSophieBase env $ pure initSt .- tx .-> expectedSt
testLEDGER initSt tx env predicateFailure@(Left _) = do
  let st = runSophieBase $ applySTSTest @(LEDGER C) (TRC (env, initSt, tx))
  st @?= predicateFailure
...
aliceGivesBobEntropic :: AliceToBob -> Tx C
aliceGivesBobEntropic
  AliceToBob
    { input,
      toBob,
      fee,
...
testProducedOverMaxWord64 =
  let biggestCoin = fromIntegral (maxBound :: Word64)
      ...
      st = runSophieBase $ applySTSTest @(LEDGER C) (TRC (ledgerEnv, (utxoState, dpState), tx))
