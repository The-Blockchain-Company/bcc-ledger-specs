...
-- |
-- Module      : Test.Sophie.Spec.Ledger.Examples.Combinators
...
import Bcc.Ledger.BaseTypes (NonNegativeInterval, Nonce (..), StrictMaybe (..), UnitInterval, (⭒))
...
import Bcc.Ledger.Sophie.Constraints (UsesTxBody, UsesTxOut)
import Bcc.Ledger.Val ((<+>), (<->))
import Bcc.Protocol.TOptimum (PoolDistr (..))
import Bcc.Protocol.TOptimum.BHeader (BHBody (..), LastAppliedBlock (..), bhHash, bhbody, lastAppliedHash, prevHashToNonce)
import Bcc.Slotting.Slot (EpochNo, WithOrigin (..))
...
import Sophie.Spec.Ledger.EpochBoundary (BlocksMade (..), SnapShot, SnapShots (..))
...
import Sophie.Spec.Ledger.PParams (PParams, PParams' (..), ProposedPPUpdates, ProtVer)
import Sophie.Spec.Ledger.STS.Chain (ChainState (..))
import Sophie.Spec.Ledger.STS.Mir (emptyInstantaneousRewards)
