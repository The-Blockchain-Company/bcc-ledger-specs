...
module Test.Sophie.Spec.Ledger.Serialisation.Tripping.JSON
  ( tests,
    ...
    prop_roundtrip_SophieGenesis_JSON,
  )
where
...
import Test.Sophie.Spec.Ledger.Serialisation.Generators.Genesis
import Test.Tasty
...
prop_roundtrip_FundPair_JSON _ =
  -- If this fails, SophieGenesis can also fail.
  Hedgehog.property $ do
...
prop_roundtrip_SophieGenesis_JSON ::
  ...
  Proxy era ->
  Property
prop_roundtrip_SophieGenesis_JSON _ = Hedgehog.withTests 500 $
  Hedgehog.property $
    do
      sg <- Hedgehog.forAll $ genSophieGenesis @era
  ...
  testGroup
    ...
    [ testProperty "Adress round trip" $
      ...
      testProperty "Genesis round trip" $
        prop_roundtrip_SophieGenesis_JSON @C Proxy,
