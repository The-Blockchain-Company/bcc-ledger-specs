...
module Test.Sophie.Spec.Ledger.Fees
  ( sizeTests,
  )
where

import Bcc.Binary (serialize)
import Bcc.Crypto.VRF (VRFAlgorithm)
import qualified Bcc.Crypto.VRF as VRF
import Bcc.Ledger.AuxiliaryData (hashAuxiliaryData)
...
-- | and has one witness
txbSimpleUTxO :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbSimpleUTxO =
  TxBody
...
-- | to use makeWitnessesVKey, we need to know we can sign the TxBody for that (SophieEra c)
type BodySignable era = DSignable (Crypto era) (Hash (Crypto era) EraIndependentTxBody)

txSimpleUTxO :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txSimpleUTxO =
  Tx
    { body = txbSimpleUTxO,
      wits =
...
-- | and has two witness
txbMutiUTxO :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbMutiUTxO =
  TxBody
    { _inputs =
        Set.fromList
...
txMutiUTxO :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txMutiUTxO =
  Tx
    { body = txbMutiUTxO,
      wits =
...
txbRegisterStake :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbRegisterStake =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
...
txRegisterStake :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txRegisterStake =
  Tx
...
txbDelegateStake :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbDelegateStake =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
...
txDelegateStake :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txDelegateStake =
  Tx
...
txbDeregisterStake :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbDeregisterStake =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
...
txDeregisterStake :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txDeregisterStake =
  Tx
...
txbRegisterPool :: Cr.Crypto c => TxBody (SophieEra c)
txbRegisterPool =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
...
txRegisterPool :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txRegisterPool =
  Tx
...
txbRetirePool :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbRetirePool =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
...
txRetirePool :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txRetirePool =
  Tx
    { body = txbRetirePool,
      wits =
...
txbWithMD :: forall c. Cr.Crypto c => TxBody (SophieEra c)
txbWithMD =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
      ...
      _ttl = SlotNo 10,
      _txUpdate = SNothing,
      _mdHash = SJust $ hashAuxiliaryData @(SophieEra c) md
    }

txWithMD :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txWithMD =
  Tx
    { body = txbWithMD,
      wits =
...
txbWithMultiSig :: Cr.Crypto c => TxBody (SophieEra c)
txbWithMultiSig =
  TxBody
...
txWithMultiSig :: forall c. Mock c => Tx (SophieEra c)
txWithMultiSig =
  Tx
        ...
        mempty
          { addrWits =
              makeWitnessesVKey
                ...
                [alicePay, bobPay],
            scriptWits = Map.singleton (hashScript @(SophieEra c) msig) msig
          },
      auxiliaryData = SNothing
    }
...
txbWithWithdrawal :: Cr.Crypto c => TxBody (SophieEra c)
txbWithWithdrawal =
  TxBody
    { _inputs = Set.fromList [TxIn genesisId 0],
      _outputs = StrictSeq.fromList [TxOut aliceAddr (Val.inject $ Coin 10)],
...
txWithWithdrawal :: forall c. (Cr.Crypto c, BodySignable (SophieEra c)) => Tx (SophieEra c)
txWithWithdrawal =
  Tx
...
testEvaluateTransactionFee =
  API.evaluateTransactionFee @(SophieEra StandardCrypto)
    pp
    txSimpleUTxONoWit
    1
