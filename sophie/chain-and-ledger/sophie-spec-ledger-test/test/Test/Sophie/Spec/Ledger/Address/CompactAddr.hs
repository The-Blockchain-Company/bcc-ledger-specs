...
module Test.Sophie.Spec.Ledger.Address.CompactAddr where

import Bcc.Ledger.Address (Addr (..), serialiseAddr)
import qualified Bcc.Ledger.Address as Addr
...
import qualified Sophie.Spec.Ledger.CompactAddr as CA
...
propDecompactSophieLazyAddr ::
  ...
  CC.Crypto crypto =>
  Gen Bool
propDecompactSophieLazyAddr = do
  stakeRef <-
    oneof
...
unsafeGetHash _ = error "this can't get a keyhash for a cole address"
