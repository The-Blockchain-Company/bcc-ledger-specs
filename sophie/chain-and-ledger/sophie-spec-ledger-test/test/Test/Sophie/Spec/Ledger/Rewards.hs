...
-- a required set of properties. It works only in the Sophie Era. It could be
-- generalized, and then moved to the Generator/Trace/ directory which computes
-- property tests in all eras.
module Test.Sophie.Spec.Ledger.Rewards (rewardTests, C, defaultMain, newEpochProp) where

import Bcc.Binary (toCBOR)
import qualified Bcc.Crypto.DSIGN as Crypto
import Bcc.Crypto.Hash (Blake2b_256, hashToBytes)
import Bcc.Crypto.Seed (mkSeedFromBytes)
import qualified Bcc.Crypto.VRF as Crypto
...
import Bcc.Ledger.Coin (Coin (..), DeltaCoin (..), rationalToCoinViaFloor, toDeltaCoin)
...
import Sophie.Spec.Ledger.API.Wallet (getRewardInfo)
...
import Test.Sophie.Spec.Ledger.Generator.Core (genCoin, genNatural)
...
maxMemberEntropic :: Integer
maxMemberEntropic = 100000

maxOwnerEntropicAbovePledge :: Integer
maxOwnerEntropicAbovePledge = 100000
...
genNonOwnerMembers = do
  ...
  fmap Map.fromList . replicateM numMembers $ do
    ...
    coins <- genCoin 0 maxMemberEntropic
...
genPoolInfo PoolSetUpArgs {poolPledge, poolCost, poolMargin, poolMembers} = do
  ...
  ownerStake <- (pledge <>) <$> genCoin 0 maxOwnerEntropicAbovePledge
...
rewardsBoundedByPot _ = property $ do
  ...
  pp <- genRewardPPs
  rewardPot <- genCoin 0 (fromIntegral $ maxEntropicSupply testGlobals)
  undelegatedEntropic <- genCoin 0 (fromIntegral $ maxEntropicSupply testGlobals)
      ...
      poolParams =
        Map.fromList $
          fmap
            ...
            pools
      totalEntropic = undelegatedEntropic <> fold stake
...
createRUpdOld ::
  ...
  EpochState era ->
  Coin ->
  SophieBase (RewardUpdateOld (Crypto era))
...
reward ::
  ...
  ActiveSlotCoeff ->
  EpochSize ->
  ProvM (KeyHashPoolProvenance crypto) SophieBase (RewardAns crypto)
