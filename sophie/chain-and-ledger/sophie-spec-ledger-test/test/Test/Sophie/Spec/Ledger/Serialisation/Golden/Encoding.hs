...
module Test.Sophie.Spec.Ledger.Serialisation.Golden.Encoding (tests) where
...
import Bcc.Crypto.DSIGN (encodeSignedDSIGN, encodeVerKeyDSIGN)
import qualified Bcc.Crypto.Hash as Monomorphic
import Bcc.Crypto.KES (SignedKES)
import Bcc.Crypto.VRF (CertifiedVRF)
...
import Bcc.Ledger.SafeHash (SafeHash, extractHash, hashAnnotated)
...
import Bcc.Ledger.Sophie.Constraints (UsesTxBody)
import Bcc.Ledger.Slot (BlockNo (..), EpochNo (..), SlotNo (..))
import Bcc.Prelude (LByteString)
...
import Sophie.Spec.Ledger.Scripts (pattern RequireSignature)
import Sophie.Spec.Ledger.Tx (Tx (..), WitnessSet, WitnessSetHKD (..), hashScript)
...
import Test.Sophie.Spec.Ledger.ConcreteCryptoTypes (C, C_Crypto, ExMock, Mock)
import Test.Sophie.Spec.Ledger.Generator.Core (PreAurum)
import Test.Sophie.Spec.Ledger.Generator.EraGen (genesisId)
import Test.Sophie.Spec.Ledger.Serialisation.GoldenUtils
  ( ToTokens (..),
    checkEncoding,
    checkEncodingCBOR,
...
testTxb :: SophieTest era => TxBody era
testTxb =
  TxBody
    Set.empty
    StrictSeq.empty
...
testKey1SigToken ::
  forall era.
  (SophieTest era, Mock (Crypto era)) =>
  Tokens ->
  Tokens
testKey1SigToken = e
  where
...
testScriptHash = hashScript @(SophieEra crypto) testScript
  ...
  testGroup
    ...
    [ checkEncodingCBOR
      ...
      let txb =
       ...
       in checkEncodingCBORAnnotated
            "tx_min"
            ( Tx @(SophieEra C_Crypto)
                txb
                (mempty {addrWits = Set.singleton w} :: Sophie.Spec.Ledger.Tx.WitnessSet C)
                SNothing
            )
            ( T (TkListLen 3)
                <> S txb
