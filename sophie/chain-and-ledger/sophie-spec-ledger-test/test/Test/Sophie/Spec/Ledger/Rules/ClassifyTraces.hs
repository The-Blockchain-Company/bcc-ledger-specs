...
module Test.Sophie.Spec.Ledger.Rules.ClassifyTraces
...
import Bcc.Ledger.BaseTypes (Globals, StrictMaybe (..), epochInfo)
...
import Bcc.Ledger.Slot (SlotNo (..), epochInfoSize)
...
import Sophie.Spec.Ledger.PParams as PParams (Update)
import Sophie.Spec.Ledger.TxBody (TxIn, Wdrl (..))
  ...
  let blockTxs :: Block era -> [Core.Tx era]
      ...
      classifications =
        [ ( "there is at least 1 certificate for every 2 transactions",
          ...
          ( "there is at least 1 Vested Delegation certificate for every 20 transactions",
            length txs < 20 * length (filter isVestedDelegation certs_),
            60
          ),
...
epochsInTrace bs =
  ...
  where
    ...
    EpochSize slotsPerEpoch = runSophieBase $ (epochInfoSize . epochInfo) testGlobals undefined
