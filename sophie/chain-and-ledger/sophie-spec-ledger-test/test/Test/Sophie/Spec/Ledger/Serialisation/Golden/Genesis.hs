...
module Test.Sophie.Spec.Ledger.Serialisation.Golden.Genesis
  ( tests,

    -- * Individual properties
    prop_golden_json_SophieGenesis,
    prop_golden_cbor_SophieGenesis,
  )
where

import Bcc.Binary (Encoding (..), ToCBOR (..), Tokens (..), serializeEncoding)
import qualified Bcc.Crypto.Hash as Hash
import Bcc.Ledger.BaseTypes (textToDns, textToUrl)
import Bcc.Ledger.Crypto (HASH)
import Bcc.Ledger.Era (Crypto (..))
import Bcc.Ledger.Keys (hashKey, hashVerKeyVRF, vKey)
...
import qualified Sophie.Spec.Ledger.API as L
import Sophie.Spec.Ledger.Genesis
...
prop_golden_json_SophieGenesis :: Property
prop_golden_json_SophieGenesis = goldenTestJSONPretty example "test/Golden/SophieGenesis"
  where
    example :: SophieGenesis (SophieEra StandardCrypto)
    example = exampleSophieGenesis

prop_golden_cbor_SophieGenesis :: Assertion
prop_golden_cbor_SophieGenesis =
...
tests =
  testGroup
    "Sophie Genesis golden tests"
    [ testProperty "SophieGenesis JSON golden test" prop_golden_json_SophieGenesis,
      testCase "SophieGenesis CBOR golden test" prop_golden_cbor_SophieGenesis
    ]
