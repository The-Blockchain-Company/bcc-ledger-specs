...
module Test.Sophie.Spec.Ledger.Rules.TestChain
  ( -- TestPoolReap
...
import Bcc.Ledger.Era (Crypto, Era, SupportsSegWit (fromTxSeq))
import Bcc.Ledger.Keys (KeyHash, KeyRole (Witness))
import Bcc.Ledger.Sophie.Constraints (UsesPParams, UsesValue)
import Bcc.Ledger.Val ((<+>), (<->))
import qualified Bcc.Ledger.Val as Val (coin)
import Bcc.Prelude (HasField (..))
...
import Sophie.Spec.Ledger.Rewards (sumRewards)
import Sophie.Spec.Ledger.STS.Chain (ChainState (..), totalBcc, totalBccPots)
import Sophie.Spec.Ledger.STS.Deleg (DelegEnv (..))
import Sophie.Spec.Ledger.STS.Ledger (LedgerEnv (..))
import Sophie.Spec.Ledger.STS.Pool (POOL, PoolEnv (..))
import Sophie.Spec.Ledger.STS.Upec (votedValue)
import Sophie.Spec.Ledger.Scripts (ScriptHash)
import Sophie.Spec.Ledger.Tx
import Sophie.Spec.Ledger.TxBody
import Sophie.Spec.Ledger.UTxO (UTxO (..), balance, totalDeposits, txins, txouts, pattern UTxO)
...
import Test.Sophie.Spec.Ledger.Generator.Block (tickChainState)
import Test.Sophie.Spec.Ledger.Generator.Core (GenEnv)
import Test.Sophie.Spec.Ledger.Generator.EraGen (EraGen (..))
import qualified Test.Sophie.Spec.Ledger.Generator.Presets as Preset (genEnv)
import Test.Sophie.Spec.Ledger.Generator.ScriptClass (scriptKeyCombinations)
...
import qualified Test.Sophie.Spec.Ledger.Rules.TestDeleg as TestDeleg
...
import qualified Test.Sophie.Spec.Ledger.Rules.TestPool as TestPool
  ( poolRegistration,
    poolRetirement,
...
-- BCC should be preserved for all state transitions in the generated trace
checkPreservation ::
  forall era.
  ( UsesValue era,
...
checkPreservation SourceSignalTarget {source, target, signal} =
  counterexample
    ( mconcat
        ( [ "\nPots before block\n",
            show (totalBccPots source),
            "\n\nPots after block\n",
            show (totalBccPots target),
            "\n\nTotal entropic before block\n",
            show sourceTotal,
            "\n\nTotal entropic after block\n",
            show targetTotal,
            "\n\nEpoch before block\n",
  ...
  where
    sourceTotal = totalBcc source
    targetTotal = totalBcc target
    ...
    rewardUpdateMsgs = case ru' of
      SNothing -> []
      SJust ru'' ->
        let ru = runSophieBase . runProvM . completeRupd $ ru''
    ...
    dispCert (DCertVested (VestedDelegCert _ _ _)) = "vested"
