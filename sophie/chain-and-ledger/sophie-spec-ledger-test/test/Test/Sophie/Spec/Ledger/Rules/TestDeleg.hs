...
module Test.Sophie.Spec.Ledger.Rules.TestDeleg
  ( keyRegistration,
    keyDeRegistration,
    keyDelegation,
    rewardsSumInvariant,
...
import Bcc.Ledger.Coin (addDeltaCoin, pattern Coin)
import qualified Bcc.Ledger.Core as Core (PParams)
...
import Sophie.Spec.Ledger.API (DELEG)
import qualified Sophie.Spec.Ledger.HardForks as HardForks (allowMIRTransfer)
          ...
          [ counterexample
            ...
            counterexample
              ...
              ( if (HardForks.allowMIRTransfer . ppDE $ denv)
                  then -- In the Aurum era, repeated fields are added

                    ( (fold $ iRReserves $ _irwd source)
                        `addDeltaCoin` (fold irwd)
                        == (fold $ (iRReserves $ _irwd target))
                    )
                  else -- Prior to the Aurum era, repeated fields overridden
