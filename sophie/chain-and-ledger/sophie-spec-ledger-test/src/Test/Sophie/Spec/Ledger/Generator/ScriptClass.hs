...
module Test.Sophie.Spec.Ledger.Generator.ScriptClass
  ( ScriptClass (..),
    Quantifier (..),
    exponential,
    anyOf,
...
import Bcc.Ledger.Keys (KeyHash, KeyPair (..), KeyRole (..), asWitness, hashKey, vKey)
import Bcc.Ledger.Sophie.Constraints (UsesScript)
...
import Sophie.Spec.Ledger.LedgerState (KeyPairs)
...
--   In post Sophie Eras, either Keylock or Require Start-Finish scripts.
baseScripts ::
  forall era.
  ScriptClass era =>
  Constants ->
