...
module Test.Sophie.Spec.Ledger.Generator.Trace.DCert
  ( CERTS,
    genDCerts,
  )
where
...
import Bcc.Ledger.Keys (HasKeyRole (coerceKeyRole), asWitness)
...
import Sophie.Spec.Ledger.Delegation.Certificates (isDeRegKey)
import Sophie.Spec.Ledger.STS.Delpl (DelplEvent, DelplPredicateFailure)
import Sophie.Spec.Ledger.TxBody (Ix)
import Sophie.Spec.Ledger.UTxO (totalDeposits)
import Test.QuickCheck (Gen)
import Test.Sophie.Spec.Ledger.Generator.Constants (Constants (..))
import Test.Sophie.Spec.Ledger.Generator.Core (GenEnv (..), KeySpace (..))
...
instance
  ...
  type BaseM (CERTS era) = SophieBase
