...
module Test.Sophie.Spec.Ledger.Generator.Trace.Chain where

-- import Test.Sophie.Spec.Ledger.Shrinkers (shrinkBlock) -- TODO FIX ME

import Bcc.Ledger.BaseTypes (UnitInterval)
import qualified Bcc.Ledger.Core as Core
import Bcc.Ledger.Era (Crypto, Era, SupportsSegWit (TxSeq))
...
import Sophie.Spec.Ledger.LedgerState (stakeDistr)
import Sophie.Spec.Ledger.STS.Bbody (BbodyEnv, BbodyState)
import qualified Sophie.Spec.Ledger.STS.Chain as STS (ChainState (ChainState))
...
-- | The first block of the Sophie era will point back to the last block of the Cole era.
...
lastColeHeaderHash :: forall proxy era. Era era => proxy era -> HashHeader (Crypto era)
lastColeHeaderHash _ = HashHeader $ mkHash 0
...
mkGenesisChainState ge@(GenEnv _ _ constants) (IRC _slotNo) = do
  ...
  pure . Right . withRewards $
    initialSophieState @era
      (At $ LastAppliedBlock (BlockNo 0) (SlotNo 0) (lastColeHeaderHash p))
      epoch0
      utxo0
      (maxLLSupply <-> Val.coin (balance utxo0))
      delegs0
      pParams
      (hashHeaderToNonce (lastColeHeaderHash p))
  where
    epoch0 = EpochNo 0
...
registerGenesisStaking ::
  ...
  ChainState era
registerGenesisStaking
  SophieGenesisStaking {sgsPools, sgsStake}
