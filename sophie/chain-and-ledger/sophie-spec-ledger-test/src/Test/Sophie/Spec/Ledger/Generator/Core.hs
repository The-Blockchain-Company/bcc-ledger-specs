...
module Test.Sophie.Spec.Ledger.Generator.Core
  ( AllIssuerKeys (..),
    applyTxBody,
    GenEnv (..),
    ScriptSpace (..),
...
import Bcc.Crypto.DSIGN.Class (DSIGNAlgorithm (..))
import qualified Bcc.Crypto.Hash as Hash
import Bcc.Crypto.VRF (evalCertified)
import Bcc.Ledger.Address (Addr (..), getRwdCred, toAddr, toCred)
...
import Bcc.Ledger.Era (Crypto (..), SupportsSegWit (hashTxSeq, toTxSeq))
import qualified Bcc.Ledger.Era as Era (TxSeq)
import Bcc.Ledger.Hashes (EraIndependentBlockBody, EraIndependentData)
...
import Bcc.Ledger.SafeHash (SafeHash, unsafeMakeSafeHash)
...
import qualified Sophie.Spec.Ledger.Tx as Ledger
...
import Test.Bcc.Crypto.VRF.Fake (WithResult (..))
...
-- | For use in the Serialisation and Example Tests, which assume Sophie, Evie, or Jen Eras.
type PreAurum era =
...
data TwoPhase3ArgInfo era = TwoPhase3ArgInfo
  { -- | A Zerepoch Script
    ...
    getData3 :: Zerepoch.Data,
    -- | A Redeemer that will make it succeed
    getRedeemer3 ::
      ( Zerepoch.Data, -- The redeeming data
...
data KeySpace era = KeySpace_
  { ksCoreNodes :: [(GenesisKeyPair (Crypto era), AllIssuerKeys (Crypto era) 'GenesisDelegate)],
    ...
    -- | Index over the cold key hashes in Vested Delegates
    ksIndexedVestedDelegates :: Map (KeyHash 'VestedDelegate (Crypto era)) (AllIssuerKeys (Crypto era) 'VestedDelegate),
...
pattern KeySpace
  ...
  where
    KeySpace ksCoreNodes ksGenesisDelegates ksCoreVested ksVestedDelegates ksStakePools ksKeyPairs ksMSigScripts =
      KeySpace_
        { ksCoreNodes,
          ksGenesisDelegates,
          ksCoreVested,
          ...
          ksIndexedVestedDelegates = mkVestedDelegatesHashMap ksCoreVested ksVestedDelegates, 
...
-- | Generate a mapping from vested delegate cold key hash to the issuer keys.
mkVestedDelegatesHashMap ::
  (CC.Crypto crypto) =>
  [(VestedKeyPair crypto, AllIssuerKeys crypto 'VestedDelegate)] ->
  [AllIssuerKeys crypto 'VestedDelegate] ->
  Map (KeyHash 'VestedDelegate crypto) (AllIssuerKeys crypto 'VestedDelegate)
mkVestedDelegatesHashMap coreVested vestedDelegates =
  Map.fromList (f <$> allDelegateKeys)
  where
    f issuerKeys = ((hashKey . vKey . cold) issuerKeys, issuerKeys)
    allDelegateKeys = (snd <$> coreVested) <> vestedDelegates
...
tooLateInEpoch s = runSophieBase $ do
  ei <- asks epochInfo
...
hashData :: forall era. Era era => Zerepoch.Data -> DataHash (Crypto era)
hashData x = unsafeMakeSafeHash (Hash.castHash (Hash.hashWith (toStrict . serialise) x))

{-
-- | Choose one of the preallocated ZerepochScripts, and return it and its Hash
genZerepoch :: forall era. GenEnv era -> Gen (Core.Script era, ScriptHash (Crypto era), TwoPhaseInfo era)
genZerepoch (GenEnv _ (ScriptSpace scripts _) _) = gettriple <$> oneof (pure <$> scripts)
  where gettriple (info@(TwoPhaseInfo script hash _data _rdmr)) = (script,hash,info)
-}

-- | Find the preallocated Script from its Hash.
findZerepoch :: forall era. Era era => GenEnv era -> (ScriptHash (Crypto era)) -> (Core.Script era, StrictMaybe (DataHash (Crypto era)))
findZerepoch (GenEnv keyspace (ScriptSpace _ _ mp3 mp2) _) hsh =
