...
module Test.Sophie.Spec.Ledger.Shrinkers where
...
shrinkTx ::
  forall era.
  (SophieTest era, Core.TxBody era ~ TxBody era) =>
  Tx era ->
  [Tx era]
...
-- NOTE ====
--
-- This is a shrinker for the Sophie TxBody, but has code in it to deal with
-- Values, which will only ever exist in the SophieMA TxBody (which is
-- different, and will have its own shrinker).
--
-- This is fine for now, but a better resolution would be: when we have a
-- shrinker for the SophieMA TxBody, let's use this logic to allow shrinking
...
shrinkTxBody (TxBody is os@((:<|) (TxOut a vs) _) cs ws tf tl tu md) =
  ...
  where
    ...
    -- put all the non-bcc tokens in the head of the outputs, append shrunk list
    mvExtraTksnToOut1 Empty = empty
    mvExtraTksnToOut1 sr =
      (TxOut a (vs <+> (extraTokens sr) <-> (Val.inject $ extraCoin sr))) <| sr

outputBalance :: SophieTest era => StrictSeq (TxOut era) -> Core.Value era
outputBalance = foldl' (\v (TxOut _ c) -> v <+> c) mempty

shrinkTxIn :: TxIn crypto -> [TxIn crypto]
shrinkTxIn = const []

shrinkTxOut :: SophieTest era => TxOut era -> [TxOut era]
