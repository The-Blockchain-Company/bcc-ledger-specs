...
module Test.Sophie.Spec.Ledger.Generator.Presets
  ( coreNodeKeys,
    keySpace,
    genEnv,
    genesisDelegs0,
...
-- NOTE: we use a seed range in the [1000...] range
-- to create keys that don't overlap with any of the other generated vested keys
coreVestedKeys ::
  CC.Crypto crypto =>
  Constants ->
  [(KeyPair 'Vested crypto, AllIssuerKeys crypto 'VestedDelegate)]
coreVestedKeys c@Constants {numCoreVested} =
  [ ( (toKeyPair . mkVestedKey) (RawSeed x 0 0 0 0),
      issuerKeys c 0 x
    )
    | x <- [1001 .. 1000 + numCoreVested]
  ]
  where
    toKeyPair (sk, vk) = KeyPair vk sk
...
-- Pre-generate a set of keys to use for Vested delegates.
vestedDelegates :: CC.Crypto crypto => Constants -> [AllIssuerKeys crypto 'VestedDelegate]
vestedDelegates c =
  [ issuerKeys c 20 x
    | x <- [0 .. 777]
  ]
