...
module Test.Sophie.Spec.Ledger.Serialisation.EraIndepGenerators
  ( mkDummyHash,
    genCoherentBlock,
    genHash,
    genSophieAddress,
...
import Bcc.Crypto.DSIGN.Mock (VerKeyDSIGN (..))
...
import Bcc.Ledger.SafeHash (HasAlgorithm, SafeHash, unsafeMakeSafeHash)
...
import Bcc.Protocol.TOptimum (IndividualPoolStake (..))
import qualified Bcc.Protocol.TOptimum.Rules.Prtcl as STS (PrtclState)
...
import Sophie.Spec.Ledger.Address.Bootstrap (ChainCode (..))
import Sophie.Spec.Ledger.EpochBoundary (BlocksMade (..))
import Sophie.Spec.Ledger.LedgerState (FutureGenDeleg, FutureVestedDeleg)
import qualified Sophie.Spec.Ledger.Metadata as MD
import Sophie.Spec.Ledger.RewardProvenance
  ( Desirability (..),
    RewardProvenance (..),
...
import qualified Sophie.Spec.Ledger.STS.Deleg as STS
import qualified Sophie.Spec.Ledger.STS.Delegs as STS
import qualified Sophie.Spec.Ledger.STS.Delpl as STS
import qualified Sophie.Spec.Ledger.STS.Ledger as STS
import qualified Sophie.Spec.Ledger.STS.Ledgers as STS
import qualified Sophie.Spec.Ledger.STS.Pool as STS
import qualified Sophie.Spec.Ledger.STS.Ppup as STS
import qualified Sophie.Spec.Ledger.STS.Tickn as STS
import qualified Sophie.Spec.Ledger.STS.Utxow as STS
import Sophie.Spec.Ledger.Tx (WitnessSetHKD (WitnessSet), hashScript)
...
import Test.Sophie.Spec.Ledger.Generator.Constants (defaultConstants)
...
import Test.Sophie.Spec.Ledger.Generator.Presets (coreNodeKeys, genEnv)
import Test.Sophie.Spec.Ledger.Serialisation.Generators.Bootstrap
  ( genBootstrapAddress,
    genSignature,
  )
...
instance CC.Crypto crypto => Arbitrary (Addr crypto) where
  arbitrary = oneof [genSophieAddress, genColeAddress]

genSophieAddress :: CC.Crypto crypto => Gen (Addr crypto)
genSophieAddress = Addr <$> arbitrary <*> arbitrary <*> arbitrary

genColeAddress :: Gen (Addr crypto)
genColeAddress = AddrBootstrap <$> genBootstrapAddress
...
instance CC.Crypto crypto => Arbitrary (FutureVestedDeleg crypto) where
  arbitrary = genericArbitraryU
  shrink = genericShrink

instance CC.Crypto crypto => Arbitrary (VestedDelegs crypto) where
  arbitrary = genericArbitraryU
  shrink = genericShrink

instance CC.Crypto crypto => Arbitrary (VestedDelegPair crypto) where
  arbitrary = genericArbitraryU
  shrink = genericShrink
...
instance CC.Crypto crypto => Arbitrary (VestedDelegCert crypto) where
  arbitrary = genericArbitraryU
  shrink = genericShrink
...
instance
  ...
  arbitrary =
    SophieGenesis
      ...
      <*> arbitrary -- sgMaxEntropicSupply
      <*> arbitrary -- sgProtocolParams
      <*> arbitrary -- sgGenDelegs
      <*> arbitrary -- sgVestedDelegs
      <*> arbitrary -- sgInitialFunds
      <*> (SophieGenesisStaking <$> arbitrary <*> arbitrary) -- sgStaking
...
-- There are tests in the shardagnostic-network repository, for instance, that
