...
import Bcc.Ledger.BaseTypes hiding (Seed)
...
import Bcc.Prelude (Natural, Word32, Word64, Word8)
...
genSophieGenesis =
  SophieGenesis
    ...
    <*> fmap Map.fromList genVestedDelegationList
    <*> fmap Map.fromList genFundsList
    <*> genStaking

genStaking :: CC.Crypto crypto => Gen (SophieGenesisStaking crypto)
genStaking =
...
genVestedDelegationList ::
  CC.Crypto crypto =>
  Gen [(KeyHash 'Vested crypto, VestedDelegPair crypto)]
genVestedDelegationList = Gen.list (Range.linear 1 10) genVestedDelegationPair

genVestedDelegationPair ::
  forall crypto.
  CC.Crypto crypto =>
  Gen (KeyHash 'Vested crypto, VestedDelegPair crypto)
genVestedDelegationPair =
  (,) <$> genKeyHash <*> (VestedDelegPair <$> genKeyHash <*> genVRFKeyHash @crypto)
...
genVestMultiple :: Gen Word64
genVestMultiple = Gen.word64 (Range.linear 1 10000)
