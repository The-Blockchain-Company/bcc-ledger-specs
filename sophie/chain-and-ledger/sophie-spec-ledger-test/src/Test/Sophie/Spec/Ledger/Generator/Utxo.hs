...
module Test.Sophie.Spec.Ledger.Generator.Utxo
  ( genTx,
    Delta (..),
    showBalance,
    getNRandomPairs,
...
import Bcc.Ledger.Val (Val (..), sumVal, (<+>), (<->), (<×>))
...
import Sophie.Spec.Ledger.STS.Delpl (DelplEnv)
...
import Test.Sophie.Spec.Ledger.Generator.Constants (Constants (..), defaultConstants)
...
-- by adding additional inputs from which more Bcc can flow into the fee.
...
deltaZero ::
  ...
  Addr (Crypto era) ->
  Delta era
deltaZero initialfee minBcc addr =
  Delta
    (initialfee <-> minBcc)
    mempty
    mempty
    (makeTxOut (Proxy @era) addr (inject minBcc))
    mempty
    mempty
...
genNextDelta
  ...
  delta@(Delta dfees extraInputs extraWitnesses change _ extraScripts) =
    let !baseTxFee = minfee pparams tx
        ...
        minBcc = getField @"_minUTxOValue" pparams
     in if remainingFee <= Coin 0 -- we've paid for all the fees
          then (pure delta) -- we're done
            ...
            if remainingFee <= (changeAmount <-> minBcc)
              then
                pure $
                  delta
                    { dfees = totalFee,
              ...
              do
                let txBody = getField @"body" tx
                    ...
                    utxo' =
                      ...
                      -- appear in inputs.
                      UTxO $
                        Map.filter (genEraGoodTxOut @era) $ -- filter out URxO entries where the TxOut are not appropriate for this Era (i.e. Keylocked in AurumEra)
                          Map.withoutKeys
                            (unUTxO utxo)
                            inputs_in_use
