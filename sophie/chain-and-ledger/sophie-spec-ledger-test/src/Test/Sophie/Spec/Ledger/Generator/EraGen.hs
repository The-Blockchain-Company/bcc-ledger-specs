...
module Test.Sophie.Spec.Ledger.Generator.EraGen
  ( genUtxo0,
    genesisId,
    EraGen (..),
    MinLEDGER_STS,
...
import Bcc.Binary (Annotator, FromCBOR, ToCBOR (toCBOR), serializeEncoding')
import qualified Bcc.Crypto.Hash as Hash
import Bcc.Ledger.Address (toAddr)
import Bcc.Ledger.AuxiliaryData (AuxiliaryDataHash, ValidateAuxiliaryData (..))
import Bcc.Ledger.BaseTypes (Network (..), SophieBase, StrictMaybe, UnitInterval)
...
import Bcc.Ledger.Pretty (PrettyA (..))
import Bcc.Ledger.SafeHash (unsafeMakeSafeHash)
import Bcc.Ledger.Sophie.Constraints (UsesPParams (..))
...
import Sophie.Spec.Ledger.PParams (ProtVer, Update)
import Sophie.Spec.Ledger.STS.Chain (CHAIN, ChainState)
import Sophie.Spec.Ledger.STS.Utxo (UtxoEnv)
import Sophie.Spec.Ledger.Tx (TxId (TxId))
...
{------------------------------------------------------------------------------
 ...
 Bcc.Ledger.Core and other modules, and by a set of Era specific generators encoded
 ...
 any Era. It should be an invariant that these properties hold for all Eras (Sophie, Evie, Jen, Aurum ...)
...
type MinLEDGER_STS era =
  ( Environment (Core.EraRule "LEDGERS" era) ~ LedgersEnv era,
    BaseM (Core.EraRule "LEDGER" era) ~ SophieBase,
    ...
    BaseM (Core.EraRule "LEDGERS" era) ~ SophieBase,
...
type MinUTXO_STS era =
  ( STS (Core.EraRule "UTXOW" era),
    BaseM (Core.EraRule "UTXOW" era) ~ SophieBase,
...
-- ======================================================================================
-- The EraGen class. Generally one method for each type family in Bcc.Ledger.Core
-- ======================================================================================
...
class
  ...
  -- use Test.Sophie.Spec.Ledger.Generator.Update(genDecentralisationParam) in your instance.
...
-- some simple scripts (NOT compound. ie either signature or Zerepoch scripts).
someScripts ::
  forall era.
  EraGen era =>
  Constants ->
...
--   Might include Keylocked scripts, Start-Finish Timelock scripts, Quantified scripts (All, Any, MofN), Zerepoch Scripts
...
allScripts c =
  (zerepochPairs genEraTwoPhase3Arg genEraTwoPhase2Arg (take 3 simple))
    ++ (take (numSimpleScripts c) simple) -- 10 means about 5% of allScripts are Zerepoch Scripts
    -- Zerepoch scripts in some Eras ([] in other Eras)
    -- [(payment,staking)] where the either payment or staking may be a zerepoch script
    ++
  ...
  where
    ...
    zerepochPairs :: [TwoPhase3ArgInfo era] -> [TwoPhase2ArgInfo era] -> [(Core.Script era, Core.Script era)] -> [(Core.Script era, Core.Script era)]
    zerepochPairs [] _ _ = []
    zerepochPairs _ [] _ = []
    zerepochPairs _ _ [] = []
    zerepochPairs args3 args2 ((pay, stake) : more) = pair : zerepochPairs args3 args2 more
      where
