...
import Sophie.Spec.Ledger.Bench.Gen
  ( genBlock,
    genTriple,
  )
import Sophie.Spec.Ledger.Bench.Rewards (createRUpd, createRUpdWithProv, genChainInEpoch)
...
import Sophie.Spec.Ledger.Rewards (likelihood)
import Sophie.Spec.Ledger.UTxO (UTxO)
import Test.QuickCheck (arbitrary)
import Test.QuickCheck.Gen as QC
import Test.Sophie.Spec.Ledger.BenchmarkFunctions
  ( initUTxO,
...
--
-- However, in the Sophie era, the value _is_ Coin, so anything generated by
...
instance CryptoClass.Crypto BenchCrypto where
  ...
  type VRF BenchCrypto = OptimumVRF
  type HASH BenchCrypto = Blake2b_256
  type ADDRHASH BenchCrypto = Blake2b_224

instance OptimumCrypto BenchCrypto

type BenchEra = SophieEra BenchCrypto
...
touchUTxOState :: Sophie.Spec.Ledger.LedgerState.UTxOState cryto -> Int
