...
module Sophie.Spec.Ledger.Bench.Rewards
  ( createRUpd,
    createRUpdWithProv,
    genChainInEpoch,
  )
...
import Sophie.Spec.Ledger.Genesis (SophieGenesisStaking (..))
...
createRUpd globals cs =
  runIdentity $
    runReaderT
      (runProvM (LS.createRUpd epochSize bm es total asc sp k))
      globals
  where
    nes = chainNes cs
    bm = LS.nesBprev nes
...
createRUpdWithProv globals cs =
  runIdentity $
    runReaderT
      (runWithProvM def (LS.createRUpd epochSize bm es total asc sp k))
      globals
  where
    nes = chainNes cs
    bm = LS.nesBprev nes
