...
the total amount of entropic in any given chain state
...
\begin{tabular}{||l|l|l|l||}\hline\hline
  ...
  utxo & circulation & s.nes.es.ls.utxoSt & Map over Entropic Values  \\ \hline
  deposits & deposits &  s.nes.es.ls.utxoSt & Entropic Value ($\Coin$) \\ \hline
  fees & fees &  s.nes.es.ls.utxoSt & Entropic Value ($\Coin$) \\ \hline
  rewards & reward accounts & s.nes.es.ls.dpstate.dstate  & Entropic Value ($\Coin$)  \\ \hline
  treasury & treasury &  s.nes.es.acnt  & Entropic Value ($\Coin$) \\ \hline
  reserves & reserves & s.nes.es.acnt & Map over Entropic Values \\ \hline
  \hline
\end{tabular}

\noindent
Notice that $\var{deposits}$, $\var{fees}$, $\var{treasury}$, and $\var{reserves}$
are all single entropic values, while $\var{utxo}$, and $\var{rewards}$ are
maps whose values are entropic.

We define the \emph{Entropic Value} of a given chain state as:
\begin{definition}[Entropic Value]
  \label{def:val}
  \begin{equation*}
    \Val(s~\in~\var{State}) =
        \Val(\var{utxo}) +
...
the \emph{Entropic Value}before the transition is the same as the
\emph{Entropic Value} after that transition.
...
\begin{lemma}
  ...
  \begin{equation*}
    e\vdash s\trans{\hyperref[fig:rules:utxo-sophie]{utxo}}{t}s'
  \end{equation*}
  then
  \begin{equation*}
    \Val(s) + w = \Val(s')
...
represents the amount of \emph{entropic} that is set aside by the system as a whole for refunding deposits.
...
%
  ...
  namely $\hyperref[fig:rules:utxo-sophie]{\mathsf{UTXO}}$.
%
  ...
  corresponds to the entropic value in $c_{i+1}$ that decays between $s_i$ and $s_{i+1}$.
  ...
  $\mathsf{UTXO}$ (Figure~\ref{fig:rules:utxo-sophie}).
...
%
  ...
  \begin{itemize}
    ...
    \item The property for $\mathsf{UTXO}$ requires a bit of explanation.
      ...
      $tx$ in $b_i$, where:
      $$
      \var{env}\vdash \var{us} \trans{\hyperref[fig:rules:utxow-sophie]{utxo}}{tx} \var{us'},
      $$
...
%
  ...
  \\~\\
  Case $\hyperref[fig:rules:utxo-sophie]{\mathsf{UTXO}}$.
...
may either be from the current epoch or an earlier one, so we split them using $\fun{sep}$:
  ...
  \end{equation*}
  Looking at the $\mathsf{UTXO}$ transition in Figure~\ref{fig:rules:utxo-sophie},
  \begin{equation*}
