...
is net neutral with respect to the Bcc in the system.
...
that Bcc is preserved by this transition.
...
\begin{figure}[ht]
  \begin{equation}\label{eq:reward-update}
    \inference[Create-Reward-Update]
    {
      ...
      ru = \Nothing
      \\~\\
      ru' \leteq \createRUpd{\SlotsPerEpoch}{b}{es}{\MaxEntropicSupply}
    }
    {
      {\begin{array}{c}
         \var{b} \\
...
\begin{figure}
  ...
  %
  \begin{align*}
      & \fun{optimumVrfChecks} \in \Seed \to \PoolDistr \to \unitIntervalNonNull \to \BHBody \to \Bool \\
      & \fun{optimumVrfChecks}~\eta_0~\var{pd}~\var{f}~\var{bhb} = \\
      & \begin{array}{cl}
...
and the Optimum blocks, depending on the overlay schedule.
...
will yield a different proportion of active slots than the Optimum blocks.
...
First we look at the regular Optimum/decentralized setting,
...
\item  Next, in the $\fun{vrfChecks}$ predicate, we check that the hash of this cold key is in the
  ...
  If $\fun{optimumVrfChecks}$ returns true, then we know that the cold key in the block header was a
...
\begin{figure}[ht]
  ...
  \begin{equation}\label{eq:decentralized}
    \inference[Decentralized]
    {
      ...
      \\~\\
      \fun{optimumVrfChecks}~\eta_0~\var{pd}~\ActiveSlotCoeff~\var{bhb}
    }
    {
      {\begin{array}{c}
         \var{d} \\
...
The protocol transition covers the common predicates of OBFT and Optimum,
...
\subsection{Cole to Sophie Transition}
\label{sec:cole-to-sophie}

This section defines the valid initial Sophie ledger states and
describes how to transition the state held by the Cole ledger to Sophie.
The Cole ledger state $\CEState$ is defined in \cite{cole_chain_spec}.
The valid initial Sophie ledger states are exactly the range of
the function $\fun{initialSophieState}$ defined in Figure~\ref{fig:functions:initial-sophie-states}.
Figure~\ref{fig:functions:to-sophie} defines the transition function from Cole.
Note that we use the hash of the final Cole header as the first evolving and
candidate nonces for Sophie.

%%
%% Figure - Sophie Initial States
%%
\begin{figure}[htb]
  \emph{Sophie Initial States}
  %
  \begin{align*}
      & \fun{initialSophieState} \in \LastAppliedBlock^? \to \Epoch \to \UTxO
        \to \Coin \to \GenesisDelegation \\
      & ~~~ \to (\Slot\mapsto\KeyHashGen^?)
        \to \PParams \to \Seed \to \ChainState \\
      & \fun{initialSophieState}~
      \left(
        \begin{array}{c}
          \var{lab} \\
          \var{e} \\
  ...
  \caption{Initial Sophie States}
  \label{fig:functions:initial-sophie-states}
\end{figure}

%%
%% Figure - Cole to Sophie State Transition
%%
\begin{figure}[htb]
  \emph{Cole to Sophie Transition}
  %
  \begin{align*}
      & \fun{toSophie} \in \CEState \to \GenesisDelegation \to \BlockNo \to \ChainState \\
      & \fun{toSophie}~
      \left(
        \begin{array}{c}
          \var{s_{last}} \\
      ...
      & ~~~~\where \\
      & ~~~~~~~~~e = \epoch{s_{last}} \\
      & ~~~~~~~~~pp = \fun{pps}~{us} \\   % this pps function is defined in the Cole chain spec
  \end{align*}

  \caption{Cole to Sophie State Transtition}
