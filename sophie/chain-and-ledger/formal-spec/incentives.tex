...
\DeliverableTitle{An Introduction to the Sophie Incentives Scheme}{Incentives Scheme Intro.}
\DeliverableResponsible{Formal Methods Team}
\EditorName{Kevin Hammond, \TBCO}
\Authors{Kevin Hammond \quad \texttt{<kevin.hammond@blockchain-company.io>}
}
...
\hypersetup{
  ...
  \begin{changelog}
        \change{2019/10/09}{Kevin Hammond}{FM (\TBCO)}{Initial Version (0.1).}
        \change{2019/10/14}{Kevin Hammond}{FM (\TBCO)}{Version 0.2.  Polished slightly}
        \change{2019/10/15}{Kevin Hammond}{FM (\TBCO)}{First draft of main net calculation.  Fixed small error in Delegator calculation.}
        \change{2019/10/16}{Kevin Hammond}{FM (\TBCO)}{Checked against design document. Further clarifications.}
        \change{2019/10/17}{Kevin Hammond}{FM (\TBCO)}{Further checking and clarification. Added images.}
        \change{2019/10/24}{Kevin Hammond}{FM (\TBCO)}{Version 0.3.  Included images from calculator.  Further checks and improvements.}
        \change{2019/11/1}{Kevin Hammond}{FM (\TBCO)}{Version 0.4.  Included suggested changes from AS. Changed $k$ to $N^{\textit{Pools}}$. Added short section on private pools.}
        \change{2019/11/26}{Kevin Hammond}{FM (\TBCO)}{Version 0.5.  Removed number of pools parameter and apparent performance for the TestNet to ensure consistency with
...
\title{An Introduction to the Sophie Incentives Scheme}

\author{Kevin Hammond  \\ {\small \texttt{kevin.hammond@blockchain-company.io}}}
...
%\textbf{\color{blue} \emph{P} } & \textbf{\color{blue}  100\% } & \textbf{\color{blue}  Participation Rate in the Sophie Incentivised TestNet } & \textbf{\color{blue}  } \\\hline
...
% %$\textit{Bcc}^{\textit{Circ}}_{\textit{Test}}$ & \BCC{31bn} @ $P=100\%$ & The \bcc{} circulating in the TestNet & $P \times \textit{Bcc}^{\textit{Circ}}$ \\\hline
%   \hline
% \end{tabular}
% \end{center}
% \caption{\bcc{} in Circulation in the Incentivised TestNet.}
% \end{figure}

% \noindent
% The \emph{participation rate} is used to calculate the total amount of \bcc{} that will circulate in the Incentivised TestNet.
% This rate is simply the percentage of the total \bcc{} in  circulation that is available to be staked in the Incentivised TestNet.
% It will be determined at the date when the
% TestNet launches and will then remain fixed.
% It is assumed that this is likely to be 20\%-50\% of the total \bcc{} in circulation.
...
% $O^{Rewards}$ & The \bcc{} rewards that are received by owner $O$ & $R^\textit{Owner} \div \textit{nOwners}$ \\\hline
% \hline
% \end{tabular}
% \end{center}
...
\begin{itemize}
  \item
    ...
    The result will be a mapping of stake keys to rewards as a whole number of Entropic.
  \item
    ...
    to Entropic amounts.  These values will be auditable and visible on the blockchain.
  ...
  \item
    ...
    All the stake keys in the snapshot will be registered by \TBCO{}. A deposit may be charged for the registration.
  \item
      The Entropic values from all valid MIR certificates will be transferred from the \bcc{} reserves
...
\clearpage
\section{The Full Incentives  Scheme (Sophie MainNet Implementation)}
\label{sec:MainNet}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.75\textwidth]{BccPerYear.pdf}
\\[2ex]
...
%% No longer relevant?
% Participation
% & Any stakeholder can participate if they hold BCC as of a specific date
% & Any stakeholder can participate if they hold BCC
%                                               \\\hline
Transaction Fees
& None
