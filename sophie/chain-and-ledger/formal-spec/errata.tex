...
We list issues found within the Sophie ledger which will be corrected in future eras.
...
This was chosen as a good value for both Shardagnostic Optimum and Shardagnostic Genesis.
...
transition while we remain using Shardagnostic Optimum, but it will be changed before
adopting Shardagnostic Genesis in the consensus layer.
...
This was corrected at the Evie hard fork.
...
\subsection{Cole redeem addresses}
\label{sec:errata:cole-redeem-addresses}

The bootstrap addresses from Figure~\ref{fig:defs:addresses} were not intended
to include the Cole era redeem addresses
(those with addrtype 2, see the Cole CDDL spec).
These addresses were, however, not spendable in the Sophie era.
At the Evie hard fork they were removed from the UTxO
and the Bcc contained in them was returned to the reserves.
