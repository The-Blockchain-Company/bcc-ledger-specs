...
module Sophie.Spec.Ledger.RewardProvenance
  ( RewardProvenance (..),
...
import Sophie.Spec.Ledger.Orphans ()
import Sophie.Spec.Ledger.TxBody (PoolParams (..), RewardAcnt (..))
...
data RewardProvenancePool crypto = RewardProvenancePool
  { -- | The number of blocks the pool produced.
    ...
    sigmaAP :: !Rational,
    -- | The number of Entropic owned by the stake pool owners.
    ...
    -- | The maximum number of Entropic this stake pool can earn.
    maxPP :: !Coin,
    -- | The stake pool's apparent performance.
    -- See Section 5.5.2 of the
    --  <https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/delegationDesignSpec/latest/download-by-type/doc-pdf/delegation_design_spec Design Specification>.
    appPerfP :: !Rational,
    -- | The total Entropic earned by the stake pool.
    poolRP :: !Coin,
    -- | The total Entropic earned by the stake pool leader.
    lRewardP :: !Coin
  }
  deriving (Eq, Generic)
...
-- <https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/specs.pool-ranking/latest/download-by-type/doc-pdf/pool-ranking stake pool ranking document> is included.
...
--  <https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/sophieLedgerSpec/latest/download-by-type/doc-pdf/ledger-spec the formal specification>.
...
data RewardProvenance crypto = RewardProvenance
  { -- | The number of slots per epoch.
    ...
    -- | The maximum Entropic supply. On mainnet, this value is equal to
    -- 100 * 10^15 (100 billion BCC).
    maxLL :: !Coin,
    -- | The maximum amount of Entropic which can be removed from the reserves
    -- to be given out as rewards for the given epoch.
    deltaR1 :: !Coin,
    -- | The difference between the total Entropic that could have been
    -- distributed as rewards this epoch (which is 'r') and what was actually distributed.
    deltaR2 :: !Coin,
    -- | The total Entropic available for rewards for the given epoch,
    -- equal to 'rPot' less 'deltaT1'.
    r :: !Coin,
    -- | The maximum Entropic supply ('maxLL') less the current value of the reserves.
    totalStake :: !Coin,
    ...
    rPot :: !Coin,
    -- | The amount of Entropic taken from the treasury for the given epoch.
    deltaT1 :: !Coin,
    -- | The amount of Entropic that is delegated during the given epoch.
    activeStake :: !Coin,
    ...
    -- See the <https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/specs.pool-ranking/latest/download-by-type/doc-pdf/pool-ranking stake pool ranking document>.
    desirabilities ::
...
instance
  ...
  where
  toCBOR (RewardProvenance p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13 p14 p15 p16 p17) =
    encode $
