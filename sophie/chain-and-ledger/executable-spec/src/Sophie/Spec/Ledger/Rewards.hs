...
module Sophie.Spec.Ledger.Rewards
  ( desirability,
    PerformanceEstimate (..),
    NonMyopic (..),
    getTopRankedPools,
...
import Quiet
import Sophie.Spec.Ledger.Delegation.PoolParams (poolSpec)
...
import Sophie.Spec.Ledger.TxBody (PoolParams (..), getRwdCred)
...
likelihood blocks t slotsPerEpoch =
  ...
  where
    ...
    --
    -- example:
    -- a pool has relative stake of 1 / 1,000,000 (~ 30k bcc of 35b bcc)
...
-- "Design Specification for Delegation and Incentives in Bcc"
desirability ::
...
--  with the Evie reward aggregation, as given by the
...
--   of "Design Specification for Delegation and Incentives in Bcc".
