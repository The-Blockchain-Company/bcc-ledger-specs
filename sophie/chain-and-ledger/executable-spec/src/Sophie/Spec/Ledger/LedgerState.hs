...
import Bcc.Ledger.Address (Addr (..), bootstrapKeyHash, isBootstrapRedeemer)
...
import Bcc.Ledger.SafeHash (HashAnnotated, extractHash, hashAnnotated)
import Bcc.Ledger.Serialization (mapFromCBOR, mapToCBOR)
...
import Sophie.Spec.Ledger.RewardProvenance (Desirability (..), RewardProvenance (..))
...
import Sophie.Spec.Ledger.Tx (extractKeyHashWitnessSet)
...
data FutureVestedDeleg crypto = FutureVestedDeleg
  { fVestedDelegSlot :: !SlotNo,
    fVestedDelegKeyHash :: !(KeyHash 'Vested crypto)
  }
  deriving (Show, Eq, Ord, Generic)
...
data DState crypto = DState
  { -- | The active reward accounts.
    ...
    -- | Future vested key delegations
    _fVestedDelegs :: !(Map (FutureVestedDeleg crypto) (VestedDelegPair crypto)),
    -- | Vested key delegations
    _vestedDelegs :: !(VestedDelegs crypto),
...
instance CC.Crypto crypto => ToCBOR (DState crypto) where
  toCBOR (DState rw dlg p fgs gs fas as ir) =
    encodeListLen 8
      <> toCBOR rw
      <> toCBOR dlg
      <> toCBOR p
...
instance CC.Crypto crypto => FromCBOR (DState crypto) where
  fromCBOR = do
    decodeRecordNamed "DState" (const 8) $ do
      rw <- fromCBOR
      dlg <- fromCBOR
      p <- fromCBOR
      fgs <- fromCBOR
      ...
      as <- fromCBOR
      ir <- fromCBOR
      pure $ DState rw dlg p fgs gs fas as ir
...
data EpochState era = EpochState
  { esAccountState :: !AccountState,
    ...
    -- See https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/specs.pool-ranking/latest/download-by-type/doc-pdf/pool-ranking
...
getGKeys nes = Map.keysSet genDelegs
  where
    ...
    LedgerState _ (DPState (DState _ _ _ _ (GenDelegs genDelegs) _ _ _) _) = ls

getAKeys ::
  NewEpochState era ->
  Set (KeyHash 'Vested (Crypto era))
getAKeys nes = Map.keysSet vestedDelegs
  where
    NewEpochState _ _ _ es _ _ = nes
    EpochState _ _ ls _ _ _ = es
    LedgerState _ (DPState (DState _ _ _ _ _ _ (VestedDelegs vestedDelegs) _) _) = ls
...
-- | Compute the entropic which are created by the transaction
produced ::
  forall era pp.
  ( Era era,
...
-- | Compute the entropic which are destroyed by the transaction
consumed ::
  forall era pp.
  ( Era era,
    HasField "certs" (Core.TxBody era) (StrictSeq (DCert (Crypto era))),
...
stakeDistr u ds ps =
  SnapShot
    ...
    poolParams
  where
    DState rewards' delegs ptrs' _ _ _ _ _= ds
...
pulseStep ::
  ...
  ProvM (RewardProvenance crypto) SophieBase (PulsingRewUpdate crypto)
...
completeRupd ::
  PulsingRewUpdate crypto ->
  ProvM (RewardProvenance crypto) SophieBase (RewardUpdate crypto)
...
createRUpd ::
  ...
  ActiveSlotCoeff ->
  Word64 ->  
  ProvM (RewardProvenance (Crypto era)) SophieBase (RewardUpdate (Crypto era))
