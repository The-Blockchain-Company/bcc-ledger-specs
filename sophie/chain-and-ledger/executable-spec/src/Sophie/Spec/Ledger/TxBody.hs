...
module Sophie.Spec.Ledger.TxBody
  ( DCert (..),
    DelegCert (..),
    Delegation (..),
    GenesisDelegCert (..),
...
import qualified Bcc.Crypto.Hash.Class as HS
...
import qualified Bcc.Ledger.Crypto as CC (ADDRHASH, Crypto)
...
import Bcc.Ledger.Val (DecodeNonNegative (..))
import Bcc.Prelude (HeapWords (..), panic)
import qualified Bcc.Prelude as HW
...
import Quiet
import Sophie.Spec.Ledger.CompactAddr
  ( CompactAddr,
    compactAddr,
    decompactAddr,
  )
...
-- assume Sophie+ type address : payment addr, staking addr (same length as payment), plus 1 word overhead
instance
...
-- | Vested key delegation certificate #TODO vested cred 
data VestedDelegCert crypto
  = VestedDelegCert 
...
data DCert crypto
  ...
  | DCertVested !(VestedDelegCert crypto)
...
instance NoThunks (VestedDelegCert crypto)
  ...
  toCBOR = \case
    ...
    DCertGenesis (GenesisDelegCert gk kh vrf) ->
      encodeListLen 4
        ...
        <> toCBOR vrf
    -- DCertVested
    DCertVested (VestedDelegCert ak kh vrf) ->
      encodeListLen 4
        <> toCBOR (6 :: Word8)
        <> toCBOR ak
        <> toCBOR kh
  ...
  fromCBOR = decodeRecordSum "DCert crypto" $
    \case
      ...
      6 -> do
        ...
        b <- fromCBOR
        c <- fromCBOR
        pure (4, DCertVested $ VestedDelegCert a b c)
      7 -> do
        x <- fromCBOR
        pure (2, DCertMir x)
      k -> invalidKey k
