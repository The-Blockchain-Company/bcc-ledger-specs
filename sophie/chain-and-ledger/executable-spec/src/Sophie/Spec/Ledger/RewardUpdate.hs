...
module Sophie.Spec.Ledger.RewardUpdate where
...
import Sophie.Spec.Ledger.RewardProvenance (RewardProvenancePool (..))
...
type Pulser c = RewardPulser c (ProvM (KeyHashPoolProvenance c) SophieBase) (RewardAns c)
...
data RewardPulser c (m :: Type -> Type) ans where
  RSLP ::
    (ans ~ RewardAns c, m ~ ProvM (KeyHashPoolProvenance c) SophieBase) =>
    !Int ->
    !(FreeVars c) ->
...
-- type:  (RewardPulser c (ProvM (KeyHashPoolProvenance c) SophieBase) (RewardAns c))
...
pulseOther :: Pulser crypto -> ProvM (RP.RewardProvenance crypto) SophieBase (Pulser crypto)
