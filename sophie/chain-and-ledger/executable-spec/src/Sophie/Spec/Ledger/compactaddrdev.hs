...
module Sophie.Spec.Ledger.CompactAddr
  ( compactAddr,
    decompactAddr,
    CompactAddr (..),
    substring,
...
import Bcc.Ledger.BaseTypes (word8ToNetwork)
...
import Bcc.Prelude (Text, cborError, panic)
...
decompactAddr (UnsafeCompactAddr bytes) =
  if testBit header cole
    then AddrBootstrap $ run "cole address" 0 bytes getBootstrapAddress
  ...
  where
    ...
    vestedcred = run "vested credential" 1 bytes $ do
      skipHash ([] @(ADDRHASH crypto))
      getVestedReference header 
