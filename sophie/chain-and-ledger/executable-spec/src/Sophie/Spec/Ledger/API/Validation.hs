...
-- | Interface to the block validation and chain extension logic in the Sophie
-- API.
module Sophie.Spec.Ledger.API.Validation
  ( ApplyBlock (..),
    applyBlock,
    applyTick,
    TickTransitionError (..),
...
import Bcc.Ledger.BaseTypes (Globals (..), SophieBase)
import Bcc.Ledger.Core (AnnotatedData, ChainData, SerialisableData)
...
import Bcc.Protocol.TOptimum.BHeader (BHeader)
...
import Sophie.Spec.Ledger.API.Protocol (OptimumCrypto)
import Sophie.Spec.Ledger.BlockChain (Block)
import Sophie.Spec.Ledger.LedgerState (NewEpochState)
import qualified Sophie.Spec.Ledger.LedgerState as LedgerState
import Sophie.Spec.Ledger.PParams (PParams' (..))
import qualified Sophie.Spec.Ledger.STS.Bbody as STS
import qualified Sophie.Spec.Ledger.STS.Chain as STS
...
class
  ( ChainData (Block era),
    ...
    BaseM (Core.EraRule "TICK" era) ~ SophieBase,
    ...
    BaseM (Core.EraRule "BBODY" era) ~ SophieBase,
...
instance OptimumCrypto crypto => ApplyBlock (SophieEra crypto)
