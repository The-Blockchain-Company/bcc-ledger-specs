...
-- | Integration between the Sophie ledger and its corresponding (Transitional
-- Optimum) protocol.
--
...
module Sophie.Spec.Ledger.API.Protocol
  ( OptimumCrypto,
    GetLedgerView (..),
    LedgerView (..),
...
import qualified Bcc.Ledger.Crypto as CC (Crypto, StandardCrypto)
...
import Bcc.Protocol.TOptimum.OCert (OCertSignable)
import qualified Bcc.Protocol.TOptimum.Rules.Prtcl as STS.Prtcl
...
import Sophie.Spec.Ledger.PParams (PParams' (..), ProtVer)
import Sophie.Spec.Ledger.STS.Chain (ChainChecksData, pparamsToChainChecksData)
import Sophie.Spec.Ledger.STS.EraMapping ()
import Sophie.Spec.Ledger.STS.Tick (TickfPredicateFailure)
import qualified Sophie.Spec.Ledger.STS.Tickn as STS.Tickn
...
instance OptimumCrypto CC.StandardCrypto
...
class
  ( ChainData (ChainDepState (Crypto era)),
    ...
    BaseM (Core.EraRule "TICKF" era) ~ SophieBase,
...
instance OptimumCrypto crypto => GetLedgerView (SophieEra crypto)

-- | Data required by the Transitional Optimum protocol from the Sophie ledger.
