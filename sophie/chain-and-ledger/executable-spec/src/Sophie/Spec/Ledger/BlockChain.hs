...
module Sophie.Spec.Ledger.BlockChain
  ( Block (Block, Block'),
    LaxBlock (..),
...
import Bcc.Ledger.Hashes (EraIndependentBlockBody)
import Bcc.Ledger.Keys (Hash, KeyHash, KeyRole (..))
...
import qualified Bcc.Protocol.TOptimum.BHeader as TP
...
import Sophie.Spec.Ledger.Tx (Tx, TxIn (..), segwitTx)
...
txSeqDecoder lax = do
  ...
  (metadata, metadataAnn) <- withSlice $
    do
      ...
      unless -- TODO this PR introduces this new test, That didn't used to run in the Sophie
...
{-# DEPRECATED HashHeader "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}

type HashHeader = TP.HashHeader

{-# DEPRECATED PrevHash "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}

type PrevHash = TP.PrevHash

{-# DEPRECATED BHBody "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}

type BHBody = TP.BHBody

{-# DEPRECATED BHeader "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}

type BHeader = TP.BHeader

{-# DEPRECATED LastAppliedBlock "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}

type LastAppliedBlock = TP.LastAppliedBlock

{-# DEPRECATED checkLeaderValue "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
checkLeaderValue ::
  forall v.
  (VRF.VRFAlgorithm v) =>
  VRF.OutputVRF v ->
...
{-# DEPRECATED bhHash "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
bhHash :: CC.Crypto crypto => TP.BHeader crypto -> TP.HashHeader crypto
bhHash = TP.bhHash

{-# DEPRECATED hashHeaderToNonce "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
hashHeaderToNonce :: TP.HashHeader crypto -> Nonce
hashHeaderToNonce = TP.hashHeaderToNonce

{-# DEPRECATED prevHashToNonce "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
prevHashToNonce :: TP.PrevHash crypto -> Nonce
prevHashToNonce = TP.prevHashToNonce

{-# DEPRECATED issuerIDfromBHBody "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
issuerIDfromBHBody :: CC.Crypto crypto => TP.BHBody crypto -> KeyHash 'BlockIssuer crypto
issuerIDfromBHBody = TP.issuerIDfromBHBody

{-# DEPRECATED bHeaderSize "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
...
{-# DEPRECATED hBbsize "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
hBbsize :: TP.BHBody crypto -> Natural
hBbsize = TP.hBbsize

{-# DEPRECATED seedEta "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
seedEta :: Nonce
seedEta = TP.seedEta

{-# DEPRECATED seedL "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
seedL :: Nonce
seedL = TP.seedL

{-# DEPRECATED mkSeed "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
mkSeed :: Nonce -> SlotNo -> Nonce -> Seed
mkSeed = TP.mkSeed

{-# DEPRECATED lastAppliedHash "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
lastAppliedHash :: WithOrigin (TP.LastAppliedBlock crypto) -> TP.PrevHash crypto
lastAppliedHash = TP.lastAppliedHash

{-# DEPRECATED bnonce "Import from Bcc.Protocol.TOptimum.BHeader instead" #-}
