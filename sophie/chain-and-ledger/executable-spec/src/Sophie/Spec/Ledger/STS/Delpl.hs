...
module Sophie.Spec.Ledger.STS.Delpl
  ( DELPL,
    DelplEnv (..),
...
import Bcc.Ledger.BaseTypes (SophieBase, invalidKey)
...
import Sophie.Spec.Ledger.STS.Deleg (DELEG, DelegEnv (..), DelegPredicateFailure)
import Sophie.Spec.Ledger.STS.Pool (POOL, PoolEnv (..), PoolPredicateFailure)
...
instance
  ...
  type BaseM (DELPL era) = SophieBase
...
delplTransition = do
  ...
  case c of
    ...
    DCertGenesis GenesisDelegCert {} -> do
      ...
      pure $ d {_dstate = ds}
    DCertVested VestedDelegCert {} -> do
      ds <-
        trans @(Core.EraRule "DELEG" era) $ TRC (DelegEnv slot ptr acnt pp, _dstate d, c)
      pure $ d {_dstate = ds}
