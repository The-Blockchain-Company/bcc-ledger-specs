...
module Sophie.Spec.Ledger.STS.Utxo
  ( UTXO,
    UtxoEnv (..),
...
import Bcc.Ledger.Keys (GenDelegs, KeyHash, KeyRole (..))
...
import Sophie.Spec.Ledger.STS.Ppup (PPUP, PPUPEnv (..), PpupEvent, PpupPredicateFailure)
import Sophie.Spec.Ledger.Tx (Tx (..), TxIn)
...
utxoInductive = do
  ...
  let outputs = Map.elems $ unUTxO (txouts txb)
      ...
      -- is correct strictly in the Sophie era (in sophieMA we would need to
