...
module Sophie.Spec.Ledger.STS.Tick
  ( TICK,
    State,
...
import Bcc.Ledger.BaseTypes (SophieBase, StrictMaybe (..), epochInfo)
...
import Bcc.Ledger.Slot (EpochNo, SlotNo, epochInfoEpoch)
...
import Sophie.Spec.Ledger.EpochBoundary (SnapShots (_pstakeMark))
...
import Sophie.Spec.Ledger.STS.NewEpoch (NEWEPOCH, NewEpochEvent, NewEpochPredicateFailure)
import Sophie.Spec.Ledger.STS.Rupd (RUPD, RupdEnv (..), RupdPredicateFailure)
...
instance
  ...
  type BaseM (TICKF era) = SophieBase
