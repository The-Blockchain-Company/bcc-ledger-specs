...
module Sophie.Spec.Ledger.STS.Epoch
  ( EPOCH,
...
import Sophie.Spec.Ledger.EpochBoundary (SnapShots, obligation)
...
import Sophie.Spec.Ledger.STS.PoolReap (POOLREAP, PoolreapEvent, PoolreapPredicateFailure, PoolreapState (..))
import Sophie.Spec.Ledger.STS.Snap (SNAP, SnapPredicateFailure)
import Sophie.Spec.Ledger.STS.Upec (UPEC, UpecPredicateFailure)
...
instance
  ...
  type BaseM (EPOCH era) = SophieBase
