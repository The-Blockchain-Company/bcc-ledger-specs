...
module Sophie.Spec.Ledger.STS.PoolReap
  ( POOLREAP,
    PoolreapEvent (..),
    PoolreapState (..),
    PredicateFailure,
...
import Bcc.Ledger.Keys (KeyHash, KeyRole (StakePool, Staking))
...
import Sophie.Spec.Ledger.TxBody (RewardAcnt, getRwdCred, _poolRAcnt)
...
instance
  ...
  type BaseM (POOLREAP era) = SophieBase
