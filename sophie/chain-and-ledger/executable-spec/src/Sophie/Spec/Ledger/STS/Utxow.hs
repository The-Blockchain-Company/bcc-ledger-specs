...
module Sophie.Spec.Ledger.STS.Utxow
  ( UTXOW,
...
import Bcc.Ledger.Rules.ValidationMode (failBecauseS, (?!#), (?!#:))
...
import Sophie.Spec.Ledger.Delegation.Certificates (isInstantaneousRewards)
...
import Sophie.Spec.Ledger.STS.Utxo (UTXO, UtxoEnv (..), UtxoEvent, UtxoPredicateFailure)
import Sophie.Spec.Ledger.Scripts (ScriptHash)
import qualified Sophie.Spec.Ledger.SoftForks as SoftForks
...
import Sophie.Spec.Ledger.TxBody (DCert, EraIndependentTxBody, TxIn, Wdrl)
import Sophie.Spec.Ledger.UTxO (UTxO, scriptsNeeded)
...
--   Note the 'embed' argument lifts from the simple Sophie (UtxowPredicateFailure) to
...
sophieStyleWitness collectVKeyWitnesses embed = do
...
instance
  ( -- Fix Core.Witnesses to the Sophie Era
    ...
    Signal (Core.EraRule "UTXO" era) ~ Core.Tx era,
    PredicateFailure (UTXOW era) ~ UtxowPredicateFailure era,
    -- Supply the HasField and Validate instances for Sophie
    SophieStyleWitnessNeeds era,
    HasField "address" (Core.TxOut era) (Addr (Crypto era))
  ) =>
  STS (UTXOW era)
