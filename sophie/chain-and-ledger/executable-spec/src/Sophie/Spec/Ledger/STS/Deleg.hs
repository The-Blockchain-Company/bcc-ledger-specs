...
module Sophie.Spec.Ledger.STS.Deleg
  ( DELEG,
    DelegEnv (..),
    PredicateFailure,
...
import Bcc.Ledger.Serialization (decodeRecordSum)
...
import Sophie.Spec.Ledger.HardForks as HardForks
...
data DelegPredicateFailure era
  ...
  | VestedKeyNotInMappingDELEG
      !(KeyHash 'Vested (Crypto era)) -- Unknown Genesis KeyHash
  | DuplicateVestedDelegateDELEG
      !(KeyHash 'VestedDelegate (Crypto era)) -- Keyhash which is already delegated to
  | InsufficientForInstantaneousRewardsDELEG
      !MIRPot -- which pot the rewards are to be drawn from, treasury or reserves
      !Coin -- amount of rewards to be given out
      !Coin -- size of the pot from which the entropic is drawn
  ...
  | DuplicateVestedVRFDELEG
      !(Hash (Crypto era) (VerKeyVRF (Crypto era)))  --VRF Keyhash already vested delegate
...
instance
  ...
  type BaseM (DELEG era) = SophieBase
...
instance
  ...
  toCBOR = \case
    ...
    VestedKeyNotInMappingDELEG akh ->
      encodeListLen 2 <> toCBOR (7 :: Word8) <> toCBOR akh
    DuplicateVestedDelegateDELEG kh ->
      encodeListLen 2 <> toCBOR (8 :: Word8) <> toCBOR kh
    InsufficientForInstantaneousRewardsDELEG pot needed potAmount ->
      encodeListLen 4 <> toCBOR (9 :: Word8)
        <> toCBOR pot
        <> toCBOR needed
        <> toCBOR potAmount
    MIRCertificateTooLateinEpochDELEG sNow sTooLate ->
      encodeListLen 3 <> toCBOR (10 :: Word8) <> toCBOR sNow <> toCBOR sTooLate
    DuplicateGenesisVRFDELEG vrf ->
      encodeListLen 2 <> toCBOR (11 :: Word8) <> toCBOR vrf
    DuplicateVestedVRFDELEG vrf ->
      encodeListLen 2 <> toCBOR (12 :: Word8) <> toCBOR vrf
    StakeKeyInRewardsDELEG cred ->
      encodeListLen 2 <> toCBOR (13 :: Word8) <> toCBOR cred
    MIRTransferNotCurrentlyAllowed ->
      encodeListLen 1 <> toCBOR (14 :: Word8)
    MIRNegativesNotCurrentlyAllowed ->
      encodeListLen 1 <> toCBOR (15 :: Word8)
    InsufficientForTransferDELEG pot needed available ->
      encodeListLen 4 <> toCBOR (16 :: Word8)
        <> toCBOR pot
        <> toCBOR needed
        <> toCBOR available
    MIRProducesNegativeUpdate ->
      encodeListLen 1 <> toCBOR (17 :: Word8)
...
instance
  ...
  fromCBOR = decodeRecordSum "PredicateFailure (DELEG era)" $
    \case
      ...
      7 -> do
        akh <- fromCBOR
        pure (2, VestedKeyNotInMappingDELEG akh)
      8 -> do
        kh <- fromCBOR
        pure (2, DuplicateVestedDelegateDELEG kh)
      9 -> do
        pot <- fromCBOR
        needed <- fromCBOR
        potAmount <- fromCBOR
      ...
      12 -> do
        vrf <- fromCBOR
        pure (2, DuplicateVestedVRFDELEG vrf)
      13 -> do
        kh <- fromCBOR
  ...
  case c of
    ...
    DCertVested (VestedDelegCert akh vkh vrf) -> do
      ...
      let acod =
          ...
          currentOtherColdKeyHashes = Set.map vestedDelegKeyHash acod
          currentOtherVrfKeyHashes = Set.map vestedDelegVrfHash acod
          futureOtherColdKeyHashes = Set.map vestedDelegKeyHash afod
          futureOtherVrfKeyHashes = Set.map vestedDelegVrfHash afod
