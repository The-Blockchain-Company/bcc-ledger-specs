...
module Sophie.Spec.Ledger.STS.Chain
  ( CHAIN,
    ChainState (..),
...
import Bcc.Protocol.TOptimum.Rules.Prtcl
  ( PRTCL,
    PrtclEnv (..),
    PrtclState (..),
    PrtlSeqFailure,
...
import Sophie.Spec.Ledger.STS.Bbody (BBODY, BbodyEnv (..), BbodyPredicateFailure, BbodyState (..))
import Sophie.Spec.Ledger.STS.Tick (TICK, TickEvent, TickPredicateFailure)
import Sophie.Spec.Ledger.STS.Tickn
...
initialSophieState ::
  ...
  Nonce ->
  ChainState era
initialSophieState lab e utxo reserves genDelegs pp initNonce =
  ChainState
    ( NewEpochState
        e
...
instance
  ...
  type BaseM (CHAIN era) = SophieBase
...
chainTransition =
  judgmentContext
    >>= \( TRC
        ...
        let LedgerState _ (DPState (DState _ _ _ _ _genDelegs _ _vestedDelegs _) (PState _ _ _)) = ls
...
-- | Calculate the total bcc pots in the epoch state
totalBccPotsES ::
  UsesValue era =>
  EpochState era ->
  BccPots
totalBccPotsES (EpochState (AccountState treasury_ reserves_) _ ls _ _ _) =
  BccPots
    { treasuryBccPot = treasury_,
      reservesBccPot = reserves_,
      rewardsBccPot = rewards_,
      utxoBccPot = circulation,
      depositsBccPot = deposits,
      feesBccPot = fees_
    }
  where
...
-- | Calculate the total bcc pots in the chain state
totalBccPots ::
  UsesValue era =>
  ChainState era ->
  BccPots
totalBccPots = totalBccPotsES . nesEs . chainNes

-- | Calculate the total bcc in the epoch state
totalBccES :: UsesValue era => EpochState era -> Coin
totalBccES cs =
  treasuryBccPot
    <> reservesBccPot
    <> rewardsBccPot
...
-- | Calculate the total bcc in the chain state
totalBcc :: UsesValue era => ChainState era -> Coin
totalBcc = totalBccES . nesEs . chainNes
