...
module Sophie.Spec.Ledger.STS.Bbody
  ( BBODY,
    BbodyState (..),
    BbodyEnv (..),
...
import Bcc.Ledger.Era (Era (Crypto), SupportsSegWit (fromTxSeq, hashTxSeq))
...
import Bcc.Ledger.Sophie.Constraints (UsesAuxiliary, UsesTxBody)
...
instance
  ...
  type BaseM (BBODY era) = SophieBase
