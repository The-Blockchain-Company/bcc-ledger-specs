...
module Bcc.Ledger.Pretty where
...
import Bcc.Ledger.Core (PParamsDelta)
...
import qualified Bcc.Ledger.Era as Era (TxSeq)
...
import Bcc.Ledger.SafeHash (SafeHash, extractHash)
...
import Bcc.Protocol.TOptimum (IndividualPoolStake (..), PoolDistr (..))
...
import Bcc.Protocol.TOptimum.OCert
  ( KESPeriod (..),
    OCert (..),
    OCertEnv (..),
    OCertSignable (..),
...
import Sophie.Spec.Ledger.Address.Bootstrap (BootstrapWitness (..), ChainCode (..))
import Sophie.Spec.Ledger.BlockChain (Block (..))
import Sophie.Spec.Ledger.CompactAddr (CompactAddr (..), decompactAddr)
...
import Sophie.Spec.Ledger.Scripts (MultiSig (..), ScriptHash (..))
...
-- ================================
-- Sophie.Spec.Ledger.STS.Chain(ChainState(..))
-- import Sophie.Spec.Ledger.BlockChain(LastAppliedBlock(..),HashHeader(..))
...
-- =================================
-- Sophie.Spec.Ledger.LedgerState.Delegation.Certificates
...
ppDState (DState r1 ds ptrs future gen futurea vested irwd) =
  ppRecord
    "DState"
    [ ("rewards", ppRewardAccounts r1),
      ...
      ("futurevesteddelegs", ppMap ppFutureVestedDeleg ppVestedDelegPair futurea),
      ("vesteddelegs", ppVestedDelegs vested),
...
ppFutureVestedDeleg :: FutureVestedDeleg crypto -> PDoc
ppFutureVestedDeleg (FutureVestedDeleg sl ah) =
  ppRecord
    "FutureVestedDeleg"
    [ ("delegSlot", ppSlotNo sl),
      ("keyhash", ppKeyHash ah)
    ]
...
instance PrettyA (FutureVestedDeleg crypto) where
  prettyA = ppFutureVestedDeleg
...
ppVestedDelegCert :: VestedDelegCert c -> PDoc
ppVestedDelegCert (VestedDelegCert a b1 c) = ppSexp "VestedDelgCert" [ppKeyHash a, ppKeyHash b1, ppHash c]
...
ppDCert (DCertVested x) = ppSexp "DCertVested" [ppVestedDelegCert x]
...
instance PrettyA (VestedDelegCert c) where
  prettyA = ppVestedDelegCert
...
ppPPUpdateEnv (PPUpdateEnv slot gd ad) =
  ppSexp
    "PPUpdateEnv"
    [ppSlotNo slot, ppGenDelegs gd, ppVestedDelegs ad]
...
ppBootstrapAddress (BootstrapAddress (Address root (Attributes (AddrAttributes path magic) y) typ)) =
  ppRecord
    "BootstrapAddress"
    [ ("root", viaShow root), -- Bcc.Crypto.Hashing.AbstractHash
      ("derivationpath", ppMaybe ppHDAddressPayload path),
      ("networkmagic", ppNetworkMagic magic),
      ("remain", ppUnparsedFields y),
      ("type", viaShow typ) -- Bcc.Chain.Common.AddrSpendingData.AddrType
    ]
...
ppAptr :: Aptr -> PDoc
ppAptr (Aptr slot n m o) = ppSexp "Aptr" [ppSlotNo slot, pretty n, pretty m, pretty o]
...
ppVestedCredential :: VestedCredential c -> PDoc
ppVestedCredential (VestedCredential (KeyHash x)) = ppSexp "VestedCredential" [ppHash x]
...
instance PrettyA (VestedCredential c) where
  prettyA = ppVestedCredential

instance PrettyA Ptr where
  prettyA = ppPtr

instance PrettyA Aptr where
  prettyA = ppAptr
  ...
  ( Globals
    ...
    ppRecord
      ...
      [ ("epochInfo", text "?"),
        ...
        ("maxEntropicSupply", pretty maxentropic),
...
ppVestedDelegPair :: VestedDelegPair c -> PDoc
ppVestedDelegPair (VestedDelegPair (KeyHash x) y) =
  ppRecord "VestedDelegPair" [("KeyHash", ppHash x), ("VrfHash", ppHash y)]

ppGKeys :: Crypto c => GKeys c -> PDoc
ppGKeys (GKeys x) = ppSexp "GKeys" [ppSet ppVKey x]

ppAKeys :: Crypto c => AKeys c -> PDoc
ppAKeys (AKeys x) = ppSexp "AKeys" [ppSet ppVKey x]
...
ppVestedDelegs :: VestedDelegs c -> PDoc
ppVestedDelegs (VestedDelegs m) = ppSexp "VestedDelegs" [ppMap ppKeyHash ppVestedDelegPair m]
...
instance Crypto c => PrettyA (AKeys c) where
  prettyA = ppAKeys

instance PrettyA (VestedDelegPair c) where
  prettyA = ppVestedDelegPair

instance PrettyA (VestedDelegs c) where
  prettyA = ppVestedDelegs
