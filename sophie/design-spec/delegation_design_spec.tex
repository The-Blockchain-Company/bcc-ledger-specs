...
\DeliverableTitle{Design Specification for Delegation and Incentives in Bcc}{Delegation/Incentives Design Spec.}
\DeliverableResponsible{Formal Methods Team}
\EditorName{Philipp Kant, \TBCO}
\Authors{Philipp Kant   \quad \texttt{<philipp.kant@blockchain-company.io>}
   \\[1em]
   Lars Br\"unjes \quad \texttt{<lars.bruenjes@blockchain-company.io>}
   \\[1em]
   Duncan Coutts  \quad \texttt{<duncan.coutts@blockchain-company.io>}
}
...
\usepackage{titlesec}
\hypersetup{
  pdftitle={Design Specification for Delegation and Incentives in Bcc},
...
\begin{changelog}
\change{2018-12-18}{PK, LB, DC}{FM (TBCO)}{First version that is considered stable enough to warrant V1. Some things still
need to be pinned down.}
\change{2019-01-07}{PK, LB, DC}{FM (TBCO)}{
...
\change{2019-01-08}{PK, LB, DC}{FM (TBCO)}{Changes after the second day of the Berlin workshop.
...
%}
\change{2019-03-01}{PK, LB, DC}{FM (TBCO)}{Incorporating further input from the workshop in Berlin, and following discussions,
...
% }
\change{2019-04-05}{PK, LB, DC}{FM (TBCO)}{Rewrote the chapter on rewards.
%
...
\change{2019-04-08}{PK, LB, DC}{FM (TBCO)}{General review of the document.
...
\change{2019-04-11}{PK, LB, DC}{FM (TBCO)}{Some subtle corrections in the rewards chapter after review by Aikaterina.
First version officially published on the TBCO blog.}
\change{2019-05-17}{PK, LB, DC}{FM (TBCO)}{Some clarifications in response to review by the auditors.}
\change{2019-06-07}{PK, LB, DC}{FM (TBCO)}{Update section on script addresses.}
\change{2019/10/09}{Kevin Hammond}{FM (TBCO)}{Added standard cover page.}
\change{2020-02-28}{PK}{FM (TBCO)}{Clarify when to use active/total stake.}
\change{2020/03/11}{DC}{FM (TBCO)}{Document the metadata feature.}
\change{2020-06-12}{PK}{FM (TBCO)}{Rewrite chapter on addresses. Now includes
  multi-sig, and is clearer about the distinction of payment addresses, stake
  addresses, credentials.}
\change{2020-06-15}{PK}{FM (TBCO)}{Ensure consistent wording, after the change
  in terminology in the last edit.}
\changePageBreak
\change{2020-07-06}{PK}{FM (TBCO)}{Correct sentence about enforcing pledge, to
...
\change{2020-07-06}{PK}{FM (TBCO)}{Minor changes after audit. Nothing that
  affects the implementation.}
\change{2020-07-23}{PK}{FM (TBCO)}{Change: undistributed rewards go to the
  reserves, not to the treasury.}
\change{2020-10-08}{J. Corduan}{FM (TBCO)}{Change: include member stake in the non-myopic
...
% \noindent
% This document provides a formal specification of the Bcc ledger for use in the Sophie implementation.
...
\author{Philipp Kant   \\ {\small \texttt{philipp.kant@blockchain-company.io}} \\
   \and Lars Br\"unjes \\ {\small \texttt{lars.bruenjes@blockchain-company.io}} \\
...
incentives mechanism to be used in the Sophie release of Bcc.
\end{abstract}
...
Delegation will allow holders of bcc to transfer their rights to participate in
...
\begin{itemize}
\item
  We cannot expect every holder of bcc to continuously run a node that
  ...
  node. Delegation allows all holders of bcc to participate in the
...
This document covers the design of necessary additions to Bcc in
...
When we make the switch from Cole (where all stake is delegated to the
nodes controlled by the Bcc Foundation, Emurgo, and TBCO) to Sophie
(where bcc holders have the freedom to control their stake), we should
...
to the core nodes of the Cole network.
...
it means to be a valid Bcc blockchain. These rules must be
...
Bcc currently has transaction fees which are intended to cover the
...
even shorter than in Cole.

\subsubsection{No lookup of old blocks}
\label{no-lookup-of-old-blocks}

The current Bcc design allows, in principle, an implementation of a
...
powerful scripting languages, such as Zerepoch and Simeon.
...
\item[Delegating from that stake address to a registered \emph{stake pool}] All
  blocks in Bcc-Sophie will be produced by a set of stake pools that need
...
In Sophie, an address has to provide information on two things: how tokens can
...
In future releases, we will add multiple languages to Bcc, with Zerepoch being
the most prominent example. In Sophie, script credentials will only be used for
the purpose of requiring signatures from multiple parties, a process known as
multi signature, or \emph{multi-sig}. For this, Bcc-Sophie will feature a
...
%
By introducing multi-sig script credentials, in Sophie, it will be possible to
...
cases of Cole and Sophie addresses. The purpose of Cole addresses is
backwards compatibility. Cole addresses have no notion of stake, so it is not
possible to delegate from a Cole address; users will have to transfer funds to
a Sophie address first.

A Sophie address contains a payment credential (again, either a key or script
credential). A transaction that consumes a UTxO entry with a Sophie address
will need a witness for its payment credential in order to be validated. In
addition, a Sophie address also contains a stake address reference. When
...
There are three options for the stake address reference in a Sophie address: it
can be provided \emph{by value}, i.e., just be the hash of a verification key or
validator script. Sophie addresses that provide their stake address reference
...
index within the transaction). Sophie addresses that contain their stake
...
system at all (just as for a Cole address). In particular, it will not be
...
bcc -- but hold it on behalf of other users -- may wish to follow a
...
exchanges, and in Bcc-Sophie, there will also be a monetary incentive to
...
\subsubsection{On Cole Addresses}
\label{bootstrap-address}

In Cole, all addresses were interpreted as
...
Bcc Foundation, Emurgo, and TBCO.

Cole addresses continue to exist in Sophie, but their
...
It is worth noting that initially, Cole addresses and enterprise addresses
have essentially identical behaviour. This might change in the future, if new
features are added to enterprise addresses.

\subsubsection{HD Wallet Structure in Sophie}
\label{hd-wallet-structure-in-sophie}

Sophie addresses with a verification key hash as payment credential
...
In addition, the leader election process in Shardagnostic Optimum requires a
...
\item
  ...
  pool operator provides to support the Bcc network.
...
registered stake addresses. For all Sophie addresses in the UTxO, the stake
...
Shardagnostic \citep{shardagnostic_classic} requires a stake distribution to
...
leader schedule and rewards for the next epoch\footnote{The detail of
  ...
  of the Shardagnostic protocol that is used. It needs to be deep enough
...
Bcc \emph{chain growth quality} is only guaranteed when for all time windows
...
\begin{itemize}
\item Perform leader election amongst the new nodes, according to Shardagnostic
  Optimum, for all \(n_s\) slots in the epoch.
\item Randomly select \(d n_s\) slots of the epoch. Let us call those the slots
  the \emph{OBFT slots} of that epoch. For the OBFT slots, the Optimum leader
  ...
  Optimum\footnote{In Shardagnostic Optimum, a large fraction of slots will deliberately
  ...
  to achieve the same block frequency as in Shardagnostic Classic, by choosing a
  shorter slot length.} (Equation (1) in \citep{shardagnostic_optimum}), and call
...
As an example\footnote{In this example, we assume \(n_s*f=21600\), to match the
  block frequency of Cole.}, consider a pool \(A\) with 1\% of stake. In the
...
i.e.~2160 active slots\footnote{assuming \(k=2160\) and an epoch length of
  \(10k\) active slots, as in Cole} will be given to pool leaders. If at least
...
\paragraph{Handling of Cole Addresses}
\label{handling-of-bootstrap-addresses}

All funds in Cole addresses will be ignored by the PoS system --
there is no stake address associated with Cole
addresses. Consequently, there will be no rewards, and stakeholders
will be incentivised to stop using Cole addresses. Our transition
...
allowing for a period where the original nodes from the Cole
...
In the Cole implementation, this is done by traversing the
...
wallet in Bcc has a fixed depth of 2. Each address in this wallet
...
operational costs or the exchange rate of bcc.
...
version of Bcc, we may facilitate this by allowing such
...
A user about to receive their first bcc (whether through redemption,
...
\item[The User Does Have a Non-Empty Wallet Already] Suppose a user
  ...
  cold wallet, such as a paper wallet. They will use Klarity to
  create this cold wallet. Klarity can offer to post the stake address
...
\item[The User Does Not Control Any Funds When Creating the Cold Wallet] In this
  case, the user will use Klarity to create a cold wallet, which will use a
  base address. Klarity will provide the stake address, including the signing
...
pool directory in Klarity (although even if they were, they would always be
...
Klarity). Optionally, the front-end could even set up (and retire) a
...
\begin{itemize}
\item
  fulfill their Shardagnostic protocol participation responsibilities,
...
mostly by \emph{monetary incentives} in the form of bcc.
...
\item
  ...
  of still available bcc that will be created per epoch.
\item
...
\begin{itemize}
\item
  The \emph{costs} of operating the pool (in bcc/epoch).
\item
...
\item
  Proof of \emph{bcc pledged to the pool}, as a list of stake addresses that
  belong to the owner(s) of the pool.
\end{itemize}

There will be no lower bound on the amount of bcc that has to be
...
Every epoch, the total amount of bcc in circulation \(T\) will be increased by
adding bcc to the rewards pot. To ensure the amount of bcc never exceeds a
...
\item[\(\eta\)] is the ratio between the number of blocks that have been produced
  during the epoch, and the expectation value\footnote{Note that in Shardagnostic
    Classic, this is just the number of slots in an epoch, but for instance in
    Optimum, where we do not always have one leader per slot, we only have a
  ...
  For Shardagnostic Classic~\citep{shardagnostic_classic}, the number of expected blocks
  will be the number of slots in an epoch, while for Shardagnostic Optimum, we will
  use the number of slots per epoch times the active slots coefficient \(f\)
  (see Equation (1) in \citep{shardagnostic_optimum}).
\item[\(\rho\)] is the monetary expansion parameter.
\item[\(T_\infty\)] is the maximal amount of bcc to ever be in circulation
  (i.e., \(45\cdot10^{9}\) bcc).
\item[T] is the amount of bcc in circulation at the beginning of the epoch for
...
\(1\), which is necessary to ensure we never exceed \(T_\infty\) bcc in
circulation.
...
\item
  higher exchange rates from bcc to USD when the system becomes more
  valuable.
\end{itemize}

Note that the fees that have been collected during the Cole era,
where all nodes have been provided by TBCO, Emurgo, and the Bcc
Foundation, have not been paid out. Those fees have reduced the amount
of bcc currently in circulation, but they did not change
...
For Bcc, we will stay close to the scheme of~\citep{bkks2018}, but we will
...
schedule, like Shardagnostic Optimum.
...
performance of the other pools and the network. We can write\footnote{We assume
  ...
  eligible to create. Strictly speaking, this is not the case for Shardagnostic
  Optimum. However, the error when linearising the leader election function is
...
In Shardagnostic Optimum, we can observe \(\sigma_a\), \(n\), and \(\Nbar\), but
...
\begin{itemize}
\item
  \(R\) are the total available rewards for the epoch (in bcc).
\item
...
\item
  ...
  owner(s) (the amount of bcc pledged during pool registration,
...
bcc is \(T=31,000,000,000\), and consider a pool whose owners
pledged bcc 15,500,000 and who attracted another bcc 15,500,000 from their
pool members. Then \[
\begin{array}{rcccl}
...
The \emph{pool operator reward \(r_\mathrm{operator}\)} (in bcc) is calculated
...
The \emph{pool member reward \(r_\mathrm{member}\)} (in bcc) is
...
\subsection{Information in Klarity}
\label{information-in-klarity}
...
Klarity will make this information readily available, as detailed in
...
want on the one hand and network efficiency and overall costs of the Bcc
...
\begin{itemize}
\item
  The expected \emph{exchange rate} \(e\) from bcc to USD (in USD/BCC).
\item
  The average \emph{costs} \(c\) (in USD) to run a pool for one year.
\item
  The average \emph{transaction fees} \(F\) (in bcc) paid during one
  epoch.
\item
  The expected ratio \(r\) of \emph{rewards} per year per staked bcc.
\item
...
\item
  \(k=100\),
\item
  \(T=31,000,000,000\,\mathrm{bcc}\),
\item
  \(T_\infty=45,000,000,000\,\mathrm{bcc}\),
\item
  \(e=0.5\,\mathrm{USD/bcc}\),
\item
  \(c=1,000\,\mathrm{USD}\),
\item
  \(F=2,000\,\mathrm{bcc}\),
...
bcc by \({1.0021}^{73}-1\approx 0.17=17\%\) per year (which sounds
pretty high\ldots).
...
\item[\cref{avoid-hard-transition} Avoid Hard Transition] As described
  ...
  transition from Cole to Sophie, with the core nodes gradually
...
One fundamental raison-d'\^{e}tre for transaction fees in Bcc (or any other
...
\begin{enumerate}
    \item
        A deposit amount (in bcc) $d_R\in(0,\infty)$ for each type of resource
...
pay an amount of $d_R$ bcc\@. When the resource is released after $t\geq 1$ slots,
...
will initially include a deposit of $d_R=2$ bcc in the transaction creating that
...
$r_R(10,000)=1.0518$ bcc will be added to the available input of the associated
transaction. The difference $d_R-r_R(10,000)=2-1.0518=0.9482$ bcc will be added
to the rewards pool of that epoch.

If our fictional user held onto the resource for 40,000 slots instead,
their refund would only be $r_R(40,000)=0.5275$ bcc, and 1.4725 bcc
would be added to the epoch rewards. In this example, refunds will
never drop below $d_{\min}\cdot d_R=0.5$ bcc.
...
not implement this design option, at least not in the initial Sophie
...
One thing to realise is that Bcc uses a sophisticated incentives scheme
...
\item When the system is initially decentralised, a majority of blocks will be
  created by the federation that ran the Cole network, and those players will
...
Adding metadata to transactions is a useful new feature in Bcc Sophie.
...
including Zerepoch Core scripts. Note again that this is only the immediate
...
such as Zerepoch scripts, in future evolutions of the system. The metadata values
